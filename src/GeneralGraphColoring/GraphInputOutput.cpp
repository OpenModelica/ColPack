...
int GraphInputOutput::ParseWidth(string FortranFormat)
{
  ...
  for (i = 0; i < LetterCount; i++)
  {
    ...
    if (PresentLetter == 'I' || PresentLetter == 'Z' || PresentLetter == 'F' || PresentLetter == 'E' ||
        PresentLetter == 'G' || PresentLetter == 'D' || PresentLetter == 'L' || PresentLetter == 'A')
    {
      FOUND = TRUE;
    }
    else if (PresentLetter == '.' || PresentLetter == ')')
    {
      FOUND = FALSE;
...
GraphInputOutput::~GraphInputOutput() { Clear(); }
...
string GraphInputOutput::GetInputFile() { return (m_s_InputFile); }
...
int GraphInputOutput::ReadMatrixMarketAdjacencyGraph(string s_InputFile, bool b_getStructureOnly)
{
  ...
  {
    printf("Warning, you tried to read matrix market matrix with values. However since graph coloring does not using "
           "matrix values but structure. We neglect the values during the reading file.\n");
  ...
  if (!in)
  {
    printf("cannot open %s", m_s_InputFile.c_str());
    exit(1);
  }
  ...
  if (b_symmetric)
  {
    do
    {
      ...
      if (rowIndex < colIndex)
      {
        printf("Error find a entry in symmetric matrix %s upper part. row %d col %d", m_s_InputFile.c_str(),
...
int GraphInputOutput::ReadHarwellBoeingAdjacencyGraph(string s_InputFile)
{
  ...
  for (i = 0; i < NROW; i++)
  {
    for (j = 0; (size_t)j < vvi_VertexAdjacency[i].size(); j++)
    {
      ...
      if (VALCRD != 0)
        m_vd_Values[m_vi_Vertices[i] + j] = vvd_Values[i][j];
    }
...
int GraphInputOutput::ReadMeTiSAdjacencyGraph(string s_InputFile)
{
  ...
  do
  {
    ...
    else
    {
      ...
      if (input_end >= 0)
      {
        while (s_InputLine[input_end] == ' ' || s_InputLine[input_end] == '\t')
          input_end--;
      }
      if (input_end < 0)
        s_InputLine = "";
      else
        s_InputLine = s_InputLine.substr(0, input_end + 1);
  ...
  do
  {
    ...
    if (i_LineCount == _FALSE)
    {
      ...
      if (vs_InputTokens.size() > 2)
      {
        ...
        else if (atoi(vs_InputTokens[2].c_str()) == 10)
        {
          i_VertexWeights = _TRUE;
        }
        else if (atoi(vs_InputTokens[2].c_str()) == 11)
        {
          i_EdgeWeights = _TRUE;
          i_VertexWeights = _TRUE;
        }
...
int GraphInputOutput::PrintGraph()
{
  ...
  if (m_vd_Values.size() > _FALSE)
  {
    ...
    cout << endl;
    cout << "[Vertices = " << STEP_DOWN(i_VertexCount) << "; Edges = " << i_EdgeCount / 2
         << "; Nonzeros = " << i_EdgeCount / 2 << "]" << endl;
    cout << endl;
  }
  else
  {
...
int GraphInputOutput::PrintGraphStructure2()
{
  ...
  cout << "Format: Vertex id (# of edges): D1 neighbor #1, D1 neighbor #2, ... (all vertices is displayed using "
          "1-based index)"
       << endl;
  cout << endl;
