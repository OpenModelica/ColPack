...
int GraphOrdering::OrderVertices(string s_OrderingVariant)
{
  ...
  else if ((s_OrderingVariant.compare("DISTANCE_TWO_LARGEST_FIRST") == 0))
  {
    return (DistanceTwoLargestFirstOrdering());
  }
  else if ((s_OrderingVariant.compare("SMALLEST_LAST_SERIAL") == 0))
  {
    return (SmallestLastOrdering_serial());
  }
  else if ((s_OrderingVariant.substr(0, 13).compare("SMALLEST_LAST") ==
            0)) // match both SMALLEST_LAST_SERIAL and SMALLEST_LAST_OMP
  {
    // cout<<"Match "<<s_OrderingVariant.substr(0,13)<<endl;
    return (SmallestLastOrdering());
  }
  else if ((s_OrderingVariant.compare("DISTANCE_TWO_SMALLEST_LAST") == 0))
  {
  ...
  else if ((s_OrderingVariant.compare("DISTANCE_TWO_INCIDENCE_DEGREE") == 0))
  {
...
int GraphOrdering::CheckVertexOrdering() { return isValidOrdering(m_vi_OrderedVertices); }
...
GraphOrdering::GraphOrdering() : GraphInputOutput() { Clear(); }

// Public Destructor 1352
GraphOrdering::~GraphOrdering() { Clear(); }
...
int GraphOrdering::printVertexEdgeMap(vector<vector<pair<int, int>>> &vvpii_VertexEdgeMap)
{
  ...
  for (int i = 0; i < (int)vvpii_VertexEdgeMap.size(); i++)
  {
    ...
    for (int j = 0; j < (int)vvpii_VertexEdgeMap[i].size(); j++)
    {
      ...
      tempS = oout.str();
      cout << setw(10) << tempS;
      if (j % 5 == 4 && j != ((int)vvpii_VertexEdgeMap[i].size()) - 1)
        cout << endl << setw(6) << ' ';
    }
    cout << endl;
  }
...
int GraphOrdering::SmallestLastOrdering() { return GraphOrdering::SmallestLastOrdering_serial(); }
//*
...
int GraphOrdering::SmallestLastOrdering_serial()
{
  ...
  i_VertexCountMinus1 =
      i_VertexCount - 1; // = i_VertexCount - 1, used when inserting selected vertices into m_vi_OrderedVertices
...
int GraphOrdering::IncidenceDegreeOrdering()
{
  ...
  vvi_GroupedIncidenceVertexDegree[i_IncidenceVertexDegree].reserve(
      (unsigned)i_VertexCount); // ONLY FOR THE FIRST BUCKET SINCE WE KNOW in THIS case
  ...
  swap(vvi_GroupedIncidenceVertexDegree[i_IncidenceVertexDegree][vi_VertexLocation[i_HighestDegreeVertex]],
       vvi_GroupedIncidenceVertexDegree[i_IncidenceVertexDegree][l]);
...
int GraphOrdering::DistanceTwoIncidenceDegreeOrdering()
{
  ...
  // initilly push all the vertices into the first bucket assuming that IncidenceVertexDegree is all 0
  vvi_GroupedInducedVertexDegree[i_InducedVertexDegree].reserve(
      (unsigned)i_VertexCount); // ONLY FOR THE FIRST BUCKET SINCE WE KNOW in THIS case
  ...
  swap(vvi_GroupedInducedVertexDegree[i_InducedVertexDegree][vi_VertexLocations[i_HighestDistanceTwoDegreeVertex]],
       vvi_GroupedInducedVertexDegree[i_InducedVertexDegree][l]);
...
string GraphOrdering::GetVertexOrderingVariant() { return (m_s_VertexOrderingVariant); }

// Public Function 1363
void GraphOrdering::GetOrderedVertices(vector<int> &output) { output = (m_vi_OrderedVertices); }

// Public Function 1364
double GraphOrdering::GetVertexOrderingTime() { return (m_d_OrderingTime); }
...
int GraphOrdering::GetMaxBackDegree()
{
  ...
  for (unsigned int i = 0; i < m_vi_Vertices.size() - 1; i++)
  {
    ...
    {
      if (vectorID2orderingID[m_vi_Edges[j]] < currentOrderingID)
        i_CurrentVertexBackDegre++;
    }
