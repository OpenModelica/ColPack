...
int GraphColoringInterface::CalculateVertexColorClasses() { return GraphColoring::CalculateVertexColorClasses(); }

void GraphColoringInterface::GetOrderedVertices(vector<int> &output) { GraphOrdering::GetOrderedVertices(output); }
...
void GraphColoringInterface::GenerateSeedHessian(double ***dp3_seed, int *ip1_SeedRowCount, int *ip1_SeedColumnCount,
                                                 string s_OrderingVariant, string s_ColoringVariant)
{
  ...
  if (s_ColoringVariant == "DISTANCE_TWO" || s_ColoringVariant == "RESTRICTED_STAR" || s_ColoringVariant == "STAR" ||
...
void GraphColoringInterface::GenerateSeedHessian_unmanaged(double ***dp3_seed, int *ip1_SeedRowCount,
                                                           int *ip1_SeedColumnCount, string s_OrderingVariant,
                                                           string s_ColoringVariant)
{
...
void GraphColoringInterface::PrintVertexEdgeMap(vector<int> &vi_Vertices, vector<int> &vi_Edges,
                                                map<int, map<int, int>> &mimi2_VertexEdgeMap)
{
  ...
  for (int i = 0; i < i_VertexCount; i++)
  {
    for (int j = vi_Vertices[i]; j < vi_Vertices[STEP_UP(i)]; j++)
    {
      if (i < vi_Edges[j])
      {
        cout << "Edge " << STEP_UP(mimi2_VertexEdgeMap[i][vi_Edges[j]]) << "\t" << " : " << STEP_UP(i) << " - "
             << STEP_UP(vi_Edges[j]) << endl;
      }
    }
  }
...
void GraphColoringInterface::PrintInducedVertexDegrees(int SetID, int i_HighestInducedVertexDegree,
...
int GraphColoringInterface::GetVertexColorCount() { return GraphColoring::GetVertexColorCount(); }
} // namespace ColPack
