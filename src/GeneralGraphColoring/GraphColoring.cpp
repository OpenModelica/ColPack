...
int GraphColoring::FindCycle(int i_Vertex, int i_AdjacentVertex, int i_DistanceOneVertex, int i_SetID,
                             vector<int> &vi_CandidateColors, vector<int> &vi_FirstVisitedOne,
                             vector<int> &vi_FirstVisitedTwo)
{
  ...
  if (i_SetID != _UNKNOWN)
  {
    ...
    else if ((i_VertexOne == i_Vertex) && (i_VertexTwo != i_AdjacentVertex))
    {
...
int GraphColoring::UpdateSet(int i_Vertex, int i_AdjacentVertex, int i_DistanceOneVertex,
                             map<int, map<int, int>> &mimi2_VertexEdgeMap, vector<int> &vi_FirstSeenOne,
                             vector<int> &vi_FirstSeenTwo, vector<int> &vi_FirstSeenThree)
{
  int i_ColorID;
...
int GraphColoring::SearchDepthFirst(int i_RootVertex, int i_ParentVertex, int i_Vertex, vector<int> &vi_TouchedVertices)
{
  ...
  for (i = m_vi_Vertices[i_Vertex]; i < m_vi_Vertices[STEP_UP(i_Vertex)]; i++)
  {
    ...
    if (m_vi_Edges[i] == i_RootVertex)
    {
      ...
      cout << "Violation " << i_ViolationCount << "\t : " << STEP_UP(i_RootVertex) << " ["
           << STEP_UP(m_vi_VertexColors[i_RootVertex]) << "] ... " << STEP_UP(i_ParentVertex) << " ["
           << STEP_UP(m_vi_VertexColors[i_ParentVertex]) << "] - " << STEP_UP(i_Vertex) << " ["
           << STEP_UP(m_vi_VertexColors[i_Vertex]) << "] - " << STEP_UP(m_vi_Edges[i]) << " ["
           << STEP_UP(m_vi_VertexColors[m_vi_Edges[i]]) << "]" << endl;
    }
    ...
    if (m_vi_VertexColors[m_vi_Edges[i]] == m_vi_VertexColors[i_Vertex])
    {
      ...
      cout << "Violation " << i_ViolationCount << "\t : " << STEP_UP(i_Vertex) << " ["
           << STEP_UP(m_vi_VertexColors[i_Vertex]) << "] - " << STEP_UP(m_vi_Edges[i]) << " ["
           << STEP_UP(m_vi_VertexColors[m_vi_Edges[i]]) << "]" << endl;
    }
...
int GraphColoring::CalculateVertexColorClasses()
{
  ...
  for (int i = 0; i < i_TotalVertexColors; i++)
  {
    ...
    else if (m_i_SmallestColorClassSize > m_vi_VertexColorFrequency[i])
    {
      m_i_SmallestColorClass = i;
...
int GraphColoring::DistanceOneColoring()
{
  ...
  */
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    ...
    cout << "DEBUG 1454 | Distance One Coloring | Coloring Vertex " << STEP_UP(i_PresentVertex) << "/" << i_VertexCount
         << endl;
...
int GraphColoring::DistanceTwoColoring()
{
  ...
  */
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    ...
    cout << "DEBUG 1455 | Distance Two Coloring | Coloring Vertex " << STEP_UP(i_PresentVertex) << "/" << i_VertexCount
         << endl;
...
int GraphColoring::NaiveStarColoring()
{
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    ...
    cout << "DEBUG 1456 | Naive Star Coloring | Coloring Vertex " << STEP_UP(i_PresentVertex) << "/" << i_VertexCount
         << endl;
...
int GraphColoring::RestrictedStarColoring()
{
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    ...
    cout << "DEBUG 1457 | Restricted Star Coloring | Coloring Vertex " << STEP_UP(i_PresentVertex) << "/"
         << i_VertexCount << endl;
    ...
    for (j = m_vi_Vertices[i_PresentVertex]; j < m_vi_Vertices[STEP_UP(i_PresentVertex)]; j++)
    {
      ...
      for (k = m_vi_Vertices[m_vi_Edges[j]]; k < m_vi_Vertices[STEP_UP(m_vi_Edges[j])]; k++)
      {
        ...
        else if (m_vi_VertexColors[m_vi_Edges[k]] < m_vi_VertexColors[m_vi_Edges[j]])
        {
          // mark as forbidden
          vi_CandidateColors[m_vi_VertexColors[m_vi_Edges[k]]] = i_PresentVertex;
        }
...
int GraphColoring::PrintPotentialHub(map<int, int> *PotentialHub_Private, int i_thread_num,
                                     pair<int, int> pii_ColorCombination)
{
  cout << "PrintPotentialHub - Star collection of combination " << pii_ColorCombination.first << " "
       << pii_ColorCombination.second << endl;
  map<int, int>::iterator mii_iter;
...
int GraphColoring::BuildStarFromColorCombination_forChecking(
    int i_Mode, int i_MaxNumThreads, int i_thread_num, pair<int, int> pii_ColorCombination,
    map<pair<int, int>, Colors2Edge_Value, lt_pii> *Colors2Edge_Private, map<int, int> *PotentialHub_Private)
{
  ...
  for (int i = 0; i < i_MaxNumThreads; i++)
  {
    ...
    if (mpii_iter != Colors2Edge_Private[i].end())
    { // Colors2Edge_Private[i] contains the color combination
      ...
      for (int j = 0; j < (int)vpii_EdgesPtr->size(); j++)
      {
        ...
        if (i_PotentialHub == 3 || b_isConflict)
        { // pii_Edge.first and pii_Edge.second are both potential hubs || conflict has been detected
          CoutLock::set();
          {
            // Detect conflict
            ...
            if (!b_isConflict)
              i_ConflictVertex = -2; // signal that both ends are Potential Hubs
            cout << "Color combination " << pii_ColorCombination.first << " " << pii_ColorCombination.second << endl;
            cout << "\t Looking at " << pii_Edge.first << "(color " << m_vi_VertexColors[pii_Edge.first] << ")-"
                 << pii_Edge.second << "(color " << m_vi_VertexColors[pii_Edge.second] << ") " << endl;
        ...
        else if (i_PotentialHub == 1)
        { // only pii_Edge.first is a potential hub
          mii_iter = PotentialHub_Private[i_thread_num].find(pii_Edge.first);
          if (mii_iter->second >= 0)
          { // This is a single edge hub => mark the pii_Edge.first vertex as hub and (the other connected vertex + pii_Edge.second) as a leaf
            PotentialHub_Private[i_thread_num][PotentialHub_Private[i_thread_num][pii_Edge.first]] =
                -(pii_Edge.first + 2);
            PotentialHub_Private[i_thread_num][pii_Edge.second] = -(pii_Edge.first + 2);
            PotentialHub_Private[i_thread_num][pii_Edge.first] = -1;
          }
          else
          { // mii_iter->second = -1 : This is a hub with more than one edge => mark pii_Edge.second as a leaf
            PotentialHub_Private[i_thread_num][pii_Edge.second] = -(pii_Edge.first + 2);
          }
        }
        else if (i_PotentialHub == 2)
        { // only pii_Edge.second is a potential hub
          mii_iter = PotentialHub_Private[i_thread_num].find(pii_Edge.second);
          if (mii_iter->second >= 0)
          { // This is a single edge hub => mark the pii_Edge.second vertex as hub and (the other connected vertex + pii_Edge.first) as a leaf
            PotentialHub_Private[i_thread_num][PotentialHub_Private[i_thread_num][pii_Edge.second]] =
                -(pii_Edge.second + 2);
            PotentialHub_Private[i_thread_num][pii_Edge.first] = -(pii_Edge.second + 2);
            PotentialHub_Private[i_thread_num][pii_Edge.second] = -1;
          }
          else
          { // mii_iter->second = -1 : This is a hub with more than one edge => mark pii_Edge.first as a leaf
            PotentialHub_Private[i_thread_num][pii_Edge.first] = -(pii_Edge.second + 2);
          }
        }
        else
        { // Both end of the vertices are seen for the first time => make them potential hubs
...
int GraphColoring::CheckStarColoring_OMP(int i_Mode, pair<int, int> *pii_ConflictColorCombination)
{
  ...
  map<pair<int, int>, Colors2Edge_Value, lt_pii> *Colors2Edge_Private =
      new map<pair<int, int>, Colors2Edge_Value,
              lt_pii>[i_MaxNumThreads]; // map 2-color combination to edges that have those 2 colors

  bool b_Stop = false;
#ifdef _OPENMP
#pragma omp parallel for default(none)                                                                                 \
    shared(i_ConflictVertex, i_VertexCount, Colors2Edge_Private, cout, i_MaxNumThreads, i_Mode, b_Stop)
#endif
  for (int i = 0; i < i_VertexCount; i++)
  {
    if (b_Stop)
    ...
    for (int j = m_vi_Vertices[i]; j < m_vi_Vertices[i + 1]; j++)
    {
      ...
      if (i < m_vi_Edges[j])
      {
        ...
        { // m_vi_VertexColors[i] == m_vi_VertexColors[ m_vi_Edges[j] ]
          // conflict found!
          CoutLock::set();
          {
            // Detect conflict
...
#pragma omp parallel default(none) firstprivate(i)                                                                     \
    shared(pii_ConflictColorCombination, i_ConflictVertex, cout, i_VertexCount, Colors2Edge_Private,                   \
               PotentialHub_Private, i_MaxNumThreads, b_Stop, i_Mode)
#endif
    for (map<pair<int, int>, Colors2Edge_Value, lt_pii>::iterator iter = Colors2Edge_Private[i].begin();
         iter != Colors2Edge_Private[i].end(); iter++)
    {
#ifdef _OPENMP
...
#endif
      {
        if (iter->second.visited == false && !b_Stop)
        {
          ...
          for (int ii = i; ii < i_MaxNumThreads; ii++)
          {
            ...
            if (iter2 != Colors2Edge_Private[ii].end())
            { // if the combination exists, we mark it as visited
          ...
          i_ConflictVertex[i_thread_num] = BuildStarFromColorCombination_forChecking(
              i_Mode, i_MaxNumThreads, i_thread_num, iter->first, Colors2Edge_Private, PotentialHub_Private);
...
#pragma omp critical
            ...
            b_Stop = true;
            cout << "IN CheckStarColoring_OMP i_ConflictVertex[" << i_thread_num
                 << "]=" << i_ConflictVertex[i_thread_num] << endl;
            // Pause();
          }
          /*
          #ifdef COLPACK_DEBUG
...
// !!! later on, remove the codes that check for conflicts (because we assume no conflict) => make this function run faster)
int GraphColoring::BuildStarFromColorCombination(int i_MaxNumThreads, int i_thread_num,
                                                 pair<int, int> pii_ColorCombination,
                                                 map<pair<int, int>, Colors2Edge_Value, lt_pii> *Colors2Edge_Private,
                                                 ...
                                                 map<int, int> *PotentialHub_Private)
{
  ...
  for (int i = 0; i < i_MaxNumThreads; i++)
  {
    ...
    { // Colors2Edge_Private[i] contains the color combination
      ...
      for (int j = 0; j < (int)vpii_EdgesPtr->size(); j++)
      {
        ...
        if (i_PotentialHub == 3 || b_isConflict)
        { // pii_Edge.first and pii_Edge.second are both potential hubs || conflict has been detected => add this edge into ConflictedEdges_Private
          CoutLock::set();
          {
            // Detect conflict
  ...
  for (; mii_iter != PotentialHub_Private[i_thread_num].end(); mii_iter++)
  {
    if (m_vi_VertexColors[mii_iter->first] == pii_ColorCombination.first)
      i_TheOtherColor = pii_ColorCombination.second;
    else
      i_TheOtherColor = pii_ColorCombination.first;
    pii_pair.first = i_TheOtherColor;
    pii_pair.second =
        mii_iter
            ->second; // if pii_pair.second < -1, then mii_iter->first is a leaf and its hub can be calculated as [-(pii_pair.second+2)]
 ...
 */
int GraphColoring::DetectConflictInColorCombination(int i_MaxNumThreads, int i_thread_num,
                                                    pair<int, int> pii_ColorCombination,
                                                    map<pair<int, int>, Colors2Edge_Value, lt_pii> *Colors2Edge_Private,
                                                    map<int, vector<pair<int, int>>> *Vertex2ColorCombination_Private,
                                                    map<int, int> *PotentialHub_Private,
...
int GraphColoring::PrintColorCombination(map<pair<int, int>, Colors2Edge_Value, lt_pii> *Colors2Edge_Private,
                                         int i_MaxNumThreads, pair<int, int> pii_ColorCombination,
                                         int i_MaxElementsOfCombination)
{
  ...
  for (int i = 0; i < i_MaxNumThreads; i++)
  {
    ...
    if (itr != Colors2Edge_Private[i].end())
    {
      ...
      for (int ii = 0; ii < (int)(*Edges).size(); ii++)
      {
        ...
        {
          cout << " MAX #=" << i_MaxElementsOfCombination
               << " REACHED. Total elements=" << i_TotalElementsOfCombination;
          break;
        }
      }
      cout << endl;
...
int GraphColoring::PrintAllColorCombination(map<pair<int, int>, Colors2Edge_Value, lt_pii> *Colors2Edge_Private,
                                            int i_MaxNumThreads, int i_MaxNumOfCombination,
                                            int i_MaxElementsOfCombination)
{
  ...
  for (int i = 0; i < i_MaxNumThreads; i++)
  {
    ...
    for (; itr != Colors2Edge_Private[i].end(); itr++)
    {
      ...
      if ((int)mpiib_VisitedColorCombination.size() >= i_MaxNumOfCombination)
        break;
    }
    if ((int)mpiib_VisitedColorCombination.size() >= i_MaxNumOfCombination)
      break;
...
int GraphColoring::PrintVertex2ColorCombination(int i_MaxNumThreads,
                                                map<int, vector<pair<int, int>>> *Vertex2ColorCombination_Private)
{
  int i_VertexCount = m_vi_Vertices.size() - 1;
...
int GraphColoring::PickVerticesToBeRecolored(int i_MaxNumThreads, vector<pair<int, int>> *ConflictedEdges_Private,
...
#pragma omp parallel for schedule(static, 1) default(none)                                                             \
    shared(cout, ConflictedEdges_Private, ConflictCount, i_MaxNumThreads)
#endif
  for (int i = 0; i < i_MaxNumThreads; i++)
  {
    for (int j = 0; j < (int)ConflictedEdges_Private[i].size(); j++)
    {
      ...
      if (m_vi_VertexColors[pii_Edge.first] == _UNKNOWN || m_vi_VertexColors[pii_Edge.second] == _UNKNOWN)
        continue;
...
#pragma omp critical
      ...
      { // ConflictCount[pii_Edge.first] == ConflictCount[pii_Edge.second]
...
int GraphColoring::BuildVertex2ColorCombination(int i_MaxNumThreads,
                                                map<int, vector<pair<int, int>>> *Vertex2ColorCombination_Private,
...
#ifdef _OPENMP
#pragma omp parallel for default(none)                                                                                 \
    shared(i_VertexCount, Vertex2ColorCombination_Private, Vertex2ColorCombination, i_MaxNumThreads)
#endif
  for (int i = 0; i < i_VertexCount; i++)
  {
    // int i_thread_num;
...
int GraphColoring::PrintVertexD1NeighborAndColor(int VertexIndex, int excludedVertex)
{
  ...
  {
    if (excludedVertex == m_vi_Edges[i])
      continue;
...
int GraphColoring::FindDistance(int v1, int v2)
{
  ...
  while (Q.size() > 0)
  {
    ...
    for (int i = m_vi_Vertices[pii_CurrentVertex.first]; i < m_vi_Vertices[pii_CurrentVertex.first + 1]; i++)
    {
      ...
      if (mib_IncludedVertices.find(i_D1Neighbor) ==
          mib_IncludedVertices.end() // make sure that i_D1Neighbor is not already included
      )
      {
...
int GraphColoring::BuildColorsSubGraph(map<int, map<int, bool>> *graph, map<int, bool> *mib_Colors)
{
  ...
  {
    if ((*mib_Colors).find(m_vi_VertexColors[i]) == (*mib_Colors).end())
      continue;
...
int GraphColoring::BuildSubGraph(map<int, map<int, bool>> *graph, int i_CenterVertex, int distance,
                                 map<int, bool> *mib_FilterByColors)
{
  ...
  for (int i = 0; i < (int)m_vi_Vertices.size() - 1; i++)
  {
    if (mib_IncludedVertices.find(i) == mib_IncludedVertices.end())
      continue;
    ...
    if (mib_FilterByColors == NULL // NOT filter by colors
        || ((*mib_FilterByColors).size() > 0 &&
            (*mib_FilterByColors).find(m_vi_VertexColors[i]) != (*mib_FilterByColors).end()) // filter by colors
    )
    {
      ...
      for (int ii = m_vi_Vertices[i]; ii < m_vi_Vertices[i + 1]; ii++)
      {
        int i_D1Neighbor = m_vi_Edges[ii];
        if (mib_FilterByColors == NULL // NOT filter by colors
            || ((*mib_FilterByColors).size() > 0 && (*mib_FilterByColors).find(m_vi_VertexColors[i_D1Neighbor]) !=
                                                        (*mib_FilterByColors).end()) // filter by colors
        )
        {
...
int GraphColoring::BuildConnectedSubGraph(map<int, map<int, bool>> *graph, int i_CenterVertex, int distance,
                                          map<int, bool> *mib_FilterByColors)
{
...
int GraphColoring::PrintVertexAndColorAdded(int i_MaxNumThreads, vector<pair<int, int>> *vi_VertexAndColorAdded,
                                            int i_LastNEntries)
{
  int i_MaxSize = vi_VertexAndColorAdded[0].size();
  for (int i = 1; i < i_MaxNumThreads; i++)
  {
    if (vi_VertexAndColorAdded[i].size() > (size_t)i_MaxSize)
      i_MaxSize = vi_VertexAndColorAdded[i].size();
  }

  if (i_LastNEntries > i_MaxSize)
    i_LastNEntries = i_MaxSize;
  ...
  for (int i = i_MaxSize - i_LastNEntries; i < i_MaxSize; i++)
  {
    ...
    for (int ii = 0; ii < i_MaxNumThreads; ii++)
    {
      ...
      cout << "(v " << setw(11) << vi_VertexAndColorAdded[ii][i].first << ",c " << setw(11)
           << vi_VertexAndColorAdded[ii][i].second << " )  ";
      //}
      // else cout<<setw(32)<<" ";
    }
    cout << endl;
...
int GraphColoring::BuildForbiddenColors(int i_MaxNumThreads, int i_thread_num, int i_CurrentVertex,
                                        map<int, bool> *mip_ForbiddenColors, map<int, int> *D1Colors,
                                        vector<map<int, int>> *Vertex2ColorCombination)
{
...
#if COLPACK_DEBUG_LEVEL > 10
        cout << flush << endl
             << "Thread " << i_thread_num << ": " << "D1 color=" << i_Color
             << "; SET count=" << D1Colors[i_thread_num][i_Color] << endl;
#endif
      }
      else
      {
        ...
        cout << flush << endl
             << "Thread " << i_thread_num << ": " << "D1 color=" << i_Color
             << "; INCREASE count=" << D1Colors[i_thread_num][i_Color] << endl;
#endif
      }
    }
  }
  ...
  for (int ii = m_vi_Vertices[i_CurrentVertex]; ii < m_vi_Vertices[i_CurrentVertex + 1]; ii++)
  {
    ...
    else
    {
      ...
      {
        if (mii_iter->second != -1)
        { // D1Neighbor is NOT a hub in the combination (m_vi_VertexColors[D1Neighbor], mii_iter->first)
          // mark mii_iter->first as forbidden
          mip_ForbiddenColors[i_thread_num][mii_iter->first] = true;
        }
      }
...
int GraphColoring::StarColoring_serial2()
{
  ...
  int i_MaxColor;
  if (m_i_VertexColorCount > 0)
    i_MaxColor = m_i_VertexColorCount;
  else
    i_MaxColor = 3;
  int i_VertexCount = m_vi_Vertices.size() - 1;
  m_vi_VertexColors.clear();
  ...
  vi_VerticesToBeColored.resize(
      i_StartingIndex[i_MaxNumThreads - 1] + vip_VerticesToBeRecolored_Private[i_MaxNumThreads - 1].size(), _UNKNOWN);
  for (int i = 0; i < i_MaxNumThreads; i++)
  {
...
#if COLPACK_DEBUG_LEVEL > 10
      cout << flush << endl
           << "degree of i_CurrentVertex " << m_vi_Vertices[i_CurrentVertex + 1] - m_vi_Vertices[i_CurrentVertex]
           << endl;
#endif
...
//*/
        ...
        if (m_vi_VertexColors[D1Neighbor] != _UNKNOWN)
        {
          ...
          else
          {
            ...
            for (; mii_iter != (*Vertex2ColorCombination)[D1Neighbor].end(); mii_iter++)
            {
              ...
              // 								}
              if (mii_iter->second < -1)
              { // D1Neighbor is a leaf in the combination (m_vi_VertexColors[D1Neighbor], mii_iter->first)
                // mark mii_iter->first as forbidden
                mip_ForbiddenColors[i_thread_num][mii_iter->first] = true;
                // 									if(i_CurrentVertex==31) {
      ...
      {
        if (mip_ForbiddenColors[i_thread_num].find(i_PotentialColor) == mip_ForbiddenColors[i_thread_num].end())
        { // if this color is not forbidden
          // see if we could get the lock for this color
          break;
        }
      }
      if (i_PotentialColor > i_MaxColor)
      { // we will need a new color
      ...
      if (false)
      {
        // #pragma omp critical
        {
          ...
          {
            // PrintVertexAndColorAdded(i_MaxNumThreads ,vi_VertexAndColorAdded);
            cout << "t" << i_thread_num << ": After assign color " << i_PotentialColor << " to v " << i_CurrentVertex
                 << endl;
            ...
            if (i_ConflictVertex >= 0)
              cout << " with color " << m_vi_VertexColors[i_ConflictVertex];
            cout << endl;
            ...
            cout << "conflict colors " << (*pii_ConflictColorCombination).first << " "
                 << (*pii_ConflictColorCombination).second << endl;
            /*
...
#if COLPACK_DEBUG_LEVEL > 10
      cout << flush << endl
           << "Thread " << i_thread_num << ": " << "Pick color " << i_PotentialColor << " for vertex "
           << i_CurrentVertex << endl
           << flush;
#endif
      ...
      for (int ii = m_vi_Vertices[i_CurrentVertex]; ii < m_vi_Vertices[i_CurrentVertex + 1]; ii++)
      {
        ...
        if (m_vi_VertexColors[D1Neighbor] != _UNKNOWN)
        {
          if (D1Colors[i_thread_num][m_vi_VertexColors[D1Neighbor]] > 1)
          {
            ...
            (*Vertex2ColorCombination)[i_CurrentVertex][m_vi_VertexColors[D1Neighbor]] =
                -1; // mark i_CurrentVertex a hub of ( m_vi_VertexColors[i_CurrentVertex] , m_vi_VertexColors[D1Neighbor] ) combination
            (*Vertex2ColorCombination)[D1Neighbor][m_vi_VertexColors[i_CurrentVertex]] = -(
                i_CurrentVertex +
                2); // mark D1Neighbor a leaf of ( m_vi_VertexColors[i_CurrentVertex] , m_vi_VertexColors[D1Neighbor] ) combination
          }
          // D1Colors[i_thread_num][ m_vi_VertexColors[D1Neighbor] ] == 1
          else if ((*Vertex2ColorCombination)[D1Neighbor].find(m_vi_VertexColors[i_CurrentVertex]) !=
                   (*Vertex2ColorCombination)[D1Neighbor].end())
          {
...
int GraphColoring::StarColoring() { return GraphColoring::StarColoring_serial2(); }
...
int GraphColoring::BuildStarCollection(vector<int> &vi_VerticesToBeRecolored)
{
  ...
  vector<int>
      vi_StarHubMap; // map a star to its hub (the center of 2-color star. For example vi_StarHubMap[star#5] = edge#7
  map<int, map<int, int>>
      mimi2_VertexEdgeMap; // map 2 vertices to its edge ID. Note that for mimi2_VertexEdgeMap[vertex#1][vertex#2]= edge#1, the id of vertex#1 must always less than vertex#2
...
int GraphColoring::PrintStarCollection(vector<int> &vi_EdgeStarMap, vector<int> &vi_StarHubMap,
...
int GraphColoring::StarColoring(vector<int> &vi_StarHubMap, vector<int> &vi_EdgeStarMap,
                                map<int, map<int, int>> &mimi2_VertexEdgeMap)
{
  int i, j, k;
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    ...
    for (j = m_vi_Vertices[i]; j < m_vi_Vertices[STEP_UP(i)]; j++)
    {
      ...
      if (i_SecondColor == i_FirstColor)
      {
        ...
        violation.push_back(i);
        violation.push_back(m_vi_Edges[j]);
        ListOfConflicts.push_back(violation);
      ...
      for (k = m_vi_Vertices[m_vi_Edges[j]]; k < m_vi_Vertices[STEP_UP(m_vi_Edges[j])]; k++)
      {
        ...
        if (i_ThirdColor == i_SecondColor)
        {
          ...
          vector<int> violation;
          violation.push_back(m_vi_Edges[j]);
          violation.push_back(m_vi_Edges[k]);
          ListOfConflicts.push_back(violation);
        ...
        if (i_ThirdColor == i_FirstColor)
        {
          for (l = m_vi_Vertices[m_vi_Edges[k]]; l < m_vi_Vertices[STEP_UP(m_vi_Edges[k])]; l++)
          {
            ...
            if (i_FourthColor == i_ThirdColor)
            {
              ...
              vector<int> violation;
              violation.push_back(m_vi_Edges[k]);
              violation.push_back(m_vi_Edges[l]);
              ListOfConflicts.push_back(violation);
            }
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    ...
    for (j = m_vi_Vertices[i]; j < m_vi_Vertices[STEP_UP(i)]; j++)
    {
      ...
      if (i_SecondColor == i_FirstColor)
      {
        ...
        cout << "Violation " << i_ViolationCount << "\t : " << STEP_UP(i) << " [" << STEP_UP(i_FirstColor) << "] - "
             << STEP_UP(m_vi_Edges[j]) << " [" << STEP_UP(i_SecondColor) << "]" << endl;
      ...
      for (k = m_vi_Vertices[m_vi_Edges[j]]; k < m_vi_Vertices[STEP_UP(m_vi_Edges[j])]; k++)
      {
        ...
        if (i_ThirdColor == i_SecondColor)
        {
          ...
          cout << "Violation " << i_ViolationCount << "\t : " << STEP_UP(m_vi_Edges[j]) << " ["
               << STEP_UP(i_SecondColor) << "] - " << STEP_UP(m_vi_Edges[k]) << " [" << STEP_UP(i_ThirdColor) << "]"
               << endl;
        ...
        if (i_ThirdColor == i_FirstColor)
        {
          for (l = m_vi_Vertices[m_vi_Edges[k]]; l < m_vi_Vertices[STEP_UP(m_vi_Edges[k])]; l++)
          {
            ...
            if (i_FourthColor == i_ThirdColor)
            {
              ...
              cout << "Violation " << i_ViolationCount << "\t : " << STEP_UP(m_vi_Edges[k]) << " ["
                   << STEP_UP(i_ThirdColor) << "] - " << STEP_UP(m_vi_Edges[l]) << " [" << STEP_UP(i_FourthColor) << "]"
                   << endl;
            }
            ...
            if (i_FourthColor == i_SecondColor)
            {
              ...
              cout << "Violation " << i_ViolationCount << "\t : " << STEP_UP(i) << " [" << STEP_UP(i_FirstColor)
                   << "] - " << STEP_UP(m_vi_Edges[j]) << " [" << STEP_UP(i_SecondColor) << "] - "
                   << STEP_UP(m_vi_Edges[k]) << " [" << STEP_UP(i_ThirdColor) << "] - " << STEP_UP(m_vi_Edges[l])
                   << " [" << STEP_UP(i_FourthColor) << "]" << endl;
...
int GraphColoring::AcyclicColoring()
{
  ...
  {
    cout << "Edge " << STEP_UP(i) << "\t" << " : " << STEP_UP(v2i_EdgeVertexMap[i][0]) << " - "
         << STEP_UP(v2i_EdgeVertexMap[i][1]) << endl;
  }
  ...
  for (i = 0; i < i_EdgeCount; i++)
  {
    cout << "Vertex " << STEP_UP(v2i_EdgeVertexMap[i][0]) << " - Vertex " << STEP_UP(v2i_EdgeVertexMap[i][1]) << "\t"
         << " : " << STEP_UP(m_mimi2_VertexEdgeMap[v2i_EdgeVertexMap[i][0]][v2i_EdgeVertexMap[i][1]]) << endl;
  }
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    ...
    cout << "DEBUG 1461 | Acyclic Coloring | Coloring Vertex " << STEP_UP(i_PresentVertex) << "/" << i_VertexCount
         << endl;
    ...
    for (j = m_vi_Vertices[i_PresentVertex]; j < m_vi_Vertices[STEP_UP(i_PresentVertex)]; j++)
    {
      ...
      for (k = m_vi_Vertices[m_vi_Edges[j]]; k < m_vi_Vertices[STEP_UP(m_vi_Edges[j])]; k++)
      {
        ...
        FindCycle(i_PresentVertex, m_vi_Edges[j], m_vi_Edges[k], i_SetID, vi_CandidateColors, vi_FirstVisitedOne,
                  vi_FirstVisitedTwo);
      }
    }
    ...
    for (j = m_vi_Vertices[i_PresentVertex]; j < m_vi_Vertices[STEP_UP(i_PresentVertex)]; j++)
    {
      ...
      i_AdjacentEdgeID = UpdateSet(i_PresentVertex, m_vi_Edges[j], i_PresentVertex, m_mimi2_VertexEdgeMap,
                                   vi_FirstSeenOne, vi_FirstSeenTwo, vi_FirstSeenThree);
      ...
      if (i_AdjacentEdgeID != _UNKNOWN)
      {
        ...
        cout << endl;
        cout << "DEBUG 1461 | Acyclic Coloring | Unify Tree | Tree " << STEP_UP(i_SetOneID) << " (Edge "
             << STEP_UP(i_EdgeID) << ") and Tree " << STEP_UP(i_SetTwoID) << " (Edge " << STEP_UP(i_AdjacentEdgeID)
             << ")" << endl;
        cout << endl;
        ...
        cout << endl;
        cout << "DEBUG 1461 | Acyclic Coloring | Unify Tree | Tree " << STEP_UP(vi_EdgeSetMap[i_EdgeID]) << " (Edge "
             << STEP_UP(i_EdgeID) << ") and Tree " << STEP_UP(vi_EdgeSetMap[i_AdjacentEdgeID]) << " (Edge "
             << STEP_UP(i_AdjacentEdgeID) << ")" << endl;
        cout << endl;
    ...
    for (j = m_vi_Vertices[i_PresentVertex]; j < m_vi_Vertices[STEP_UP(i_PresentVertex)]; j++)
    {
      ...
      for (k = m_vi_Vertices[m_vi_Edges[j]]; k < m_vi_Vertices[STEP_UP(m_vi_Edges[j])]; k++)
      {
        ...
        if (m_vi_VertexColors[m_vi_Edges[k]] == m_vi_VertexColors[i_PresentVertex])
        {
          ...
          i_EdgeID = UpdateSet(i_PresentVertex, m_vi_Edges[j], m_vi_Edges[k], m_mimi2_VertexEdgeMap, vi_FirstSeenOne,
                               vi_FirstSeenTwo, vi_FirstSeenThree);
...
#if DEBUG == 1461
            cout << endl;
            cout << "DEBUG 1461 | Acyclic Coloring | Unify Tree | Tree " << STEP_UP(vi_EdgeSetMap[i_EdgeID])
                 << " (Edge " << STEP_UP(i_EdgeID) << ") and Tree " << STEP_UP(vi_EdgeSetMap[i_AdjacentEdgeID])
                 << " (Edge " << STEP_UP(i_AdjacentEdgeID) << ")" << endl;
            cout << endl;
...
// GraphColoringInterface::PrintVertexEdgeMap(m_vi_Vertices, m_vi_Edges, m_mimi2_VertexEdgeMap);
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    for (j = m_vi_Vertices[i]; j < m_vi_Vertices[STEP_UP(i)]; j++)
    {
      if (i < m_vi_Edges[j])
      {
        cout << "Edge " << STEP_UP(m_mimi2_VertexEdgeMap[i][m_vi_Edges[j]]) << "\t" << " : " << STEP_UP(i) << " - "
             << STEP_UP(m_vi_Edges[j]) << endl;
      }
    }
  }
...
// cout<<"*1"<<endl;
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    ...
    cout << "DEBUG 1462 | Acyclic Coloring | Coloring Vertex " << STEP_UP(i_PresentVertex) << "/" << i_VertexCount
         << endl;
    ...
    for (j = m_vi_Vertices[i_PresentVertex]; j < m_vi_Vertices[STEP_UP(i_PresentVertex)]; j++)
    {
      ...
      if (i_AdjacentEdgeID != _UNKNOWN)
      {
        ...
        cout << endl;
        cout << "DEBUG 1462 | Acyclic Coloring | Unify Tree | Tree " << STEP_UP(i_SetOneID) << " (Edge "
             << STEP_UP(i_EdgeID) << ") and Tree " << STEP_UP(i_SetTwoID) << " (Edge " << STEP_UP(i_AdjacentEdgeID)
             << ")" << endl;
        cout << endl;
        ...
        cout << endl;
        cout << "DEBUG 1462 | Acyclic Coloring | Unify Tree | Tree " << STEP_UP(vi_EdgeSetMap[i_EdgeID]) << " (Edge "
             << STEP_UP(i_EdgeID) << ") and Tree " << STEP_UP(vi_EdgeSetMap[i_AdjacentEdgeID]) << " (Edge "
             << STEP_UP(i_AdjacentEdgeID) << ")" << endl;
        cout << endl;
      ...
      for (k = m_vi_Vertices[m_vi_Edges[j]]; k < m_vi_Vertices[STEP_UP(m_vi_Edges[j])]; k++)
      {
        ...
        if (m_vi_VertexColors[m_vi_Edges[k]] == m_vi_VertexColors[i_PresentVertex])
        {
          ...
          if (i_EdgeID != _UNKNOWN)
          {
            ...
            cout << endl;
            cout << "DEBUG 1462 | Acyclic Coloring | Unify Tree | Tree " << STEP_UP(vi_EdgeSetMap[i_EdgeID])
                 << " (Edge " << STEP_UP(i_EdgeID) << ") and Tree " << STEP_UP(vi_EdgeSetMap[i_AdjacentEdgeID])
                 << " (Edge " << STEP_UP(i_AdjacentEdgeID) << ")" << endl;
            cout << endl;
...
int GraphColoring::AcyclicColoring(vector<int> &vi_Sets, map<int, vector<int>> &mivi_VertexSets)
{
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    for (j = m_vi_Vertices[i]; j < m_vi_Vertices[STEP_UP(i)]; j++)
    {
      if (i < m_vi_Edges[j])
      {
        cout << "Edge " << STEP_UP(mimi2_VertexEdgeMap[i][m_vi_Edges[j]]) << "\t" << " : " << STEP_UP(i) << " - "
             << STEP_UP(m_vi_Edges[j]) << endl;
      }
    }
  }
...
// cout<<"*1"<<endl;
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    ...
    for (j = m_vi_Vertices[i_PresentVertex]; j < m_vi_Vertices[STEP_UP(i_PresentVertex)]; j++)
    {
      ...
      // cout<<"*2"<<endl;
      i_AdjacentEdgeID = UpdateSet(i_PresentVertex, m_vi_Edges[j], i_PresentVertex, mimi2_VertexEdgeMap,
                                   vi_FirstSeenOne, vi_FirstSeenTwo, vi_FirstSeenThree);
    ...
    for (j = m_vi_Vertices[i_PresentVertex]; j < m_vi_Vertices[STEP_UP(i_PresentVertex)]; j++)
    {
      ...
      for (k = m_vi_Vertices[m_vi_Edges[j]]; k < m_vi_Vertices[STEP_UP(m_vi_Edges[j])]; k++)
      {
        ...
        if (m_vi_VertexColors[m_vi_Edges[k]] == m_vi_VertexColors[i_PresentVertex])
        {
          ...
          i_EdgeID = UpdateSet(i_PresentVertex, m_vi_Edges[j], m_vi_Edges[k], mimi2_VertexEdgeMap, vi_FirstSeenOne,
                               vi_FirstSeenTwo, vi_FirstSeenThree);
...
int GraphColoring::TriangularColoring()
{
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    ...
    cout << "DEBUG 1464 | Triangular Coloring | Processing Vertex " << STEP_UP(i_PresentVertex) << "/" << i_VertexCount
         << endl;
    ...
    for (j = m_vi_Vertices[i_PresentVertex]; j < m_vi_Vertices[STEP_UP(i_PresentVertex)]; j++)
    {
      ...
      for (k = m_vi_Vertices[m_vi_Edges[j]]; k < m_vi_Vertices[STEP_UP(m_vi_Edges[j])]; k++)
      {
        ...
        if ((vi_VertexHierarchy[m_vi_Edges[j]] > vi_VertexHierarchy[i_PresentVertex]) &&
            (vi_VertexHierarchy[m_vi_Edges[j]] > vi_VertexHierarchy[m_vi_Edges[k]]))
        {
          _FOUND = _FALSE;
...
int GraphColoring::ModifiedTriangularColoring()
{
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    ...
    cout << "DEBUG 1465 | Triangular Coloring | Coloring Vertex " << STEP_UP(i_PresentVertex) << "/" << i_VertexCount
         << endl;
...
int GraphColoring::CheckTriangularColoring() { return (CheckAcyclicColoring()); }

// Public Function 1467
string GraphColoring::GetVertexColoringVariant() { return (m_s_VertexColoringVariant); }
...
int GraphColoring::GetVertexColorCount() { return (STEP_UP(m_i_VertexColorCount)); }

// Public Function 1469
void GraphColoring::GetVertexColors(vector<int> &output) { output = (m_vi_VertexColors); }
...
double GraphColoring::GetVertexColoringTime() { return (m_d_ColoringTime); }

// Public Function 1473
double GraphColoring::GetVertexColoringCheckingTime() { return (m_d_CheckingTime); }
...
int GraphColoring::PrintVertexColors()
{
  ...
  cout << endl;
  cout << m_s_VertexColoringVariant << " Coloring | " << m_s_VertexOrderingVariant << " Ordering | Vertex Colors | "
       << m_s_InputFile << endl;
  cout << endl;
  ...
  {
    cout << endl;
    cout << "[Total Colors = " << STEP_UP(m_i_VertexColorCount) << "; Total Stars = " << m_i_ColoringUnits << "]"
         << endl;
    cout << "[Ordering Time = " << m_d_OrderingTime << "; Coloring Time = " << m_d_ColoringTime << "]" << endl;
    cout << endl;
  }
  else if (m_s_VertexColoringVariant.compare("ACYCLIC") == 0)
  {
    cout << endl;
    cout << "[Total Colors = " << STEP_UP(m_i_VertexColorCount) << "; Total Sets = " << m_i_ColoringUnits << "]"
         << endl;
    cout << "[Ordering Time = " << m_d_OrderingTime << "; Coloring Time = " << m_d_ColoringTime << "]" << endl;
    cout << endl;
  }
  else if (m_s_VertexColoringVariant.compare("TRIANGULAR") == 0)
  {
    cout << endl;
  ...
  if (m_s_VertexColoringVariant.compare("TRIANGULAR") == 0)
  {
    cout << endl;
    cout << "[Total Colors = " << STEP_UP(m_i_VertexColorCount) << "]" << endl;
    cout << "[Ordering Time = " << m_d_OrderingTime << "; Sequencing Time = " << m_d_SequencingTime
         << "; Coloring Time = " << m_d_ColoringTime << "]" << endl;
    cout << endl;
  }
  else
  {
...
int GraphColoring::FileVertexColors()
{
  ...
  else if (m_s_VertexOrderingVariant.compare("DISTANCE_TWO_LARGEST_FIRST") == 0)
  {
  ...
  else if (m_s_VertexOrderingVariant.compare("DISTANCE_TWO_SMALLEST_LAST") == 0)
  {
  ...
  else if (m_s_VertexOrderingVariant.compare("DISTANCE_TWO_INCIDENCE_DEGREE") == 0)
  {
  ...
  else if (m_s_VertexColoringVariant.compare("DISTANCE_TWO") == 0)
  {
    s_ColoringExtension = ".D2.";
  }
  else if (m_s_VertexColoringVariant.compare("NAIVE_STAR") == 0)
  {
    s_ColoringExtension = ".NS.";
  }
  else if (m_s_VertexColoringVariant.compare("RESTRICTED_STAR") == 0)
  {
    s_ColoringExtension = ".RS.";
  }
  else if (m_s_VertexColoringVariant.compare("STAR") == 0)
  {
  ...
  OutputStream << endl;
  OutputStream << m_s_VertexColoringVariant << " Coloring | " << m_s_VertexOrderingVariant
               << " Ordering | Vertex Colors | " << m_s_InputFile << endl;
  OutputStream << endl;
  ...
  {
    OutputStream << endl;
    OutputStream << "[Total Colors = " << STEP_UP(m_i_VertexColorCount) << "; Total Stars = " << m_i_ColoringUnits
                 << "]" << endl;
  ...
  {
    OutputStream << endl;
    OutputStream << "[Total Colors = " << STEP_UP(m_i_VertexColorCount) << "; Total Sets = " << m_i_ColoringUnits << "]"
                 << endl;
    OutputStream << "[Ordering Time = " << m_d_OrderingTime << "; Coloring Time = " << m_d_ColoringTime << "]" << endl;
    OutputStream << endl;
  }
  ...
  if (m_s_VertexColoringVariant.compare("TRIANGULAR") == 0)
  {
    OutputStream << endl;
    OutputStream << "[Total Colors = " << STEP_UP(m_i_VertexColorCount) << "]" << endl;
    OutputStream << "[Ordering Time = " << m_d_OrderingTime << "; Sequencing Time = " << m_d_SequencingTime
                 << "; Coloring Time = " << m_d_ColoringTime << "]" << endl;
    OutputStream << endl;
  }
  else
...
{
  cout << endl;
  cout << m_s_VertexColoringVariant << " Coloring | " << m_s_VertexOrderingVariant << " Ordering | " << m_s_InputFile
       << endl;
  cout << endl;
  ...
  if (m_s_VertexColoringVariant.compare("STAR") == 0)
  {
    ...
    cout << "[Total Colors = " << STEP_UP(m_i_VertexColorCount) << "; Total Stars = " << m_i_ColoringUnits << "]"
         << endl;
    cout << "[Vertex Count = " << STEP_DOWN(m_vi_Vertices.size()) << "; Edge Count = " << m_vi_Edges.size() / 2 << "]"
         << endl;
    cout << "[Ordering Time = " << m_d_OrderingTime << "; Coloring Time = " << m_d_ColoringTime << "]" << endl;
    cout << endl;
  }
  ...
  else if (m_s_VertexColoringVariant.compare("TRIANGULAR") == 0)
  {
    cout << endl;
    cout << "[Total Colors = " << STEP_UP(m_i_VertexColorCount) << "]" << endl;
    cout << "[Vertex Count = " << STEP_DOWN(m_vi_Vertices.size()) << "; Edge Count = " << m_vi_Edges.size() << "]"
         << endl;
    cout << "[Ordering Time = " << m_d_OrderingTime << "; Coloring Time = " << m_d_ColoringTime << "]" << endl;
    cout << endl;
  }
...
int GraphColoring::FileVertexColoringMetrics()
{
  ...
  else if (m_s_VertexOrderingVariant.compare("NATURAL") == 0)
  {
    s_OrderingExtension = ".N.";
  }
  else if (m_s_VertexOrderingVariant.compare("LARGEST FIRST") == 0)
  {
    s_OrderingExtension = ".LF.";
  }
  else if (m_s_VertexOrderingVariant.compare("DISTANCE TWO LARGEST FIRST") == 0)
  {
    s_OrderingExtension = ".D2LF.";
  }
  else if (m_s_VertexOrderingVariant.compare("SMALLEST LAST") == 0)
  {
    s_OrderingExtension = ".SL.";
  }
  else if (m_s_VertexOrderingVariant.compare("DISTANCE TWO SMALLEST LAST") == 0)
  {
    s_OrderingExtension = ".D2SL.";
  }
  else if (m_s_VertexOrderingVariant.compare("INCIDENCE DEGREE") == 0)
  {
    s_OrderingExtension = ".ID.";
  }
  else if (m_s_VertexOrderingVariant.compare("DISTANCE TWO INCIDENCE DEGREE") == 0)
  {
    s_OrderingExtension = ".D2ID.";
  }
  else
  ...
  else if (m_s_VertexColoringVariant.compare("DISTANCE ONE") == 0)
  {
    s_ColoringExtension = ".D1.";
  }
  else if (m_s_VertexColoringVariant.compare("DISTANCE TWO") == 0)
  {
    s_ColoringExtension = ".D2.";
  }
  else if (m_s_VertexColoringVariant.compare("NAIVE STAR") == 0)
  {
    s_ColoringExtension = ".NS.";
  }
  else if (m_s_VertexColoringVariant.compare("RESTRICTED STAR") == 0)
  {
    s_ColoringExtension = ".RS.";
  }
  else if (m_s_VertexColoringVariant.compare("STAR") == 0)
  ...
  OutputStream << endl;
  OutputStream << m_s_VertexColoringVariant << " Coloring | " << m_s_VertexOrderingVariant << " Ordering | "
               << m_s_InputFile << endl;
  OutputStream << endl;
  ...
  if (m_s_VertexColoringVariant.compare("STAR") == 0)
  {
    ...
    OutputStream << "[Total Colors = " << STEP_UP(m_i_VertexColorCount) << "; Total Stars = " << m_i_ColoringUnits
                 << "]" << endl;
    OutputStream << "[Vertex Count = " << STEP_DOWN(m_vi_Vertices.size()) << "; Edge Count = " << m_vi_Edges.size()
                 << "]" << endl;
    OutputStream << "[Ordering Time = " << m_d_OrderingTime << "; Coloring Time = " << m_d_ColoringTime << "]" << endl;
    OutputStream << endl;
  }
...
void GraphColoring::PrintVertexColorClasses()
{
  ...
  {
    cout << endl;
    cout << "Vertex Color Classes | " << m_s_VertexColoringVariant << " Coloring | " << m_s_VertexOrderingVariant
         << " Ordering | " << m_s_InputFile << " | Vertex Colors Not Set" << endl;
    cout << endl;
  ...
  cout << endl;
  cout << "[Largest Color Class : " << STEP_UP(m_i_LargestColorClass)
       << "; Largest Color Class Size : " << m_i_LargestColorClassSize << "]" << endl;
  cout << "[Smallest Color Class : " << STEP_UP(m_i_SmallestColorClass)
       << "; Smallest Color Class Size : " << m_i_SmallestColorClassSize << "]" << endl;
...
double **GraphColoring::GetSeedMatrix_unmanaged(int *i_SeedRowCount, int *i_SeedColumnCount)
{
  ...
  for (int i = 0; i < i_size; i++)
  {
    ...
    for (int j = 0; j < i_num_of_colors; j++)
      Seed[i][j] = 0.;
  }
...
int GraphColoring::CheckQuickDistanceTwoColoring(int Verbose)
{
  ...
  cout << "VertexWithMaxDegree = " << i_VertexWithMaxDegree << "; MaximumVertexDegree = " << i_MaximumVertexDegree
       << endl;
  if (Verbose < 2)
    return 1;
    ...
    for (int j = i + 1; j < m_vi_Vertices[i_VertexWithMaxDegree + 1]; j++)
    {
      if (m_vi_VertexColors[m_vi_Edges[i]] == m_vi_VertexColors[m_vi_Edges[j]])
        printf("\t m_vi_VertexColors[m_vi_Edges[i(%d)](%d)](%d) == m_vi_VertexColors[m_vi_Edges[j(%d)](%d)](%d)\n", i,
               m_vi_Edges[i], m_vi_VertexColors[m_vi_Edges[i]], j, m_vi_Edges[j], m_vi_VertexColors[m_vi_Edges[j]]);
    }
  }
  ...
  for (i_PresentVertex = 0; i_PresentVertex < i_VertexCount; i_PresentVertex++)
  {
    ...
    for (j = m_vi_Vertices[i_PresentVertex]; j < m_vi_Vertices[STEP_UP(i_PresentVertex)]; j++)
    {
      ...
      if (m_vi_VertexColors[i_PresentVertex] == m_vi_VertexColors[i_DistanceOneVertex])
      {
        ...
        if (Verbose < 1)
          return 1;
        printf("D1 VIOLATION. m_vi_VertexColors[i_PresentVertex(%d)](%d) == "
               "m_vi_VertexColors[i_DistanceOneVertex(%d)](%d) \n",
               i_PresentVertex, m_vi_VertexColors[i_PresentVertex], i_DistanceOneVertex,
               m_vi_VertexColors[i_DistanceOneVertex]);
        if (Verbose < 2)
          return 1;
      }
      ...
      for (k = m_vi_Vertices[i_DistanceOneVertex]; k < m_vi_Vertices[STEP_UP(i_DistanceOneVertex)]; k++)
      {
        ...
        if (i_DistanceTwoVertex == i_PresentVertex)
          continue; // We don't want to make a loop. Ignore this case
        ...
        if (m_vi_VertexColors[i_PresentVertex] == m_vi_VertexColors[i_DistanceTwoVertex])
        {
          ...
          if (Verbose < 1)
            return 1;
          printf("D2 VIOLATION. m_vi_VertexColors[i_PresentVertex(%d)](%d) == "
                 "m_vi_VertexColors[i_DistanceTwoVertex(%d)](%d) \n",
                 i_PresentVertex, m_vi_VertexColors[i_PresentVertex], i_DistanceTwoVertex,
                 m_vi_VertexColors[i_DistanceTwoVertex]);
...
void GraphColoring::SetStringVertexColoringVariant(string s) { m_s_VertexColoringVariant = s; }

void GraphColoring::SetVertexColorCount(int i_VertexColorCount) { m_i_VertexColorCount = i_VertexColorCount; }

#ifndef _OPENMP
// Public Function
int GraphColoring::D1_Coloring_OMP()
{
  printf("OpenMP is disabled. Recompile the code with correct flag\n");
  return _TRUE;
}
#endif
...
#pragma omp parallel
  ...
  if (randValues == nullptr)
  {
    printf("Not enough memory for array of %ld doubles\n", NVer);
    exit(1);
  }
  int seed = 12345;
  srand(seed);
...
#pragma omp parallel for
    for (long Qi = 0; Qi < QTail; Qi++)
    {
      ...
      for (long k = adj1; k < adj2; k++)
      {
        ...
        {
          if ((randValues[v] < randValues[verInd[k]]) || ((randValues[v] == randValues[verInd[k]]) && (v < verInd[k])))
          {
            long whereInQ;
#pragma omp critical
            whereInQ = QtmpTail++;
