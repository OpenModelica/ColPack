...
int GraphCore::GetVertexCount() { return (STEP_DOWN(m_vi_Vertices.size())); }

// Public Function 1104
int GraphCore::GetEdgeCount() { return (m_vi_Edges.size() / 2); }

// Public Function 1105
int GraphCore::GetMaximumVertexDegree() { return (m_i_MaximumVertexDegree); }

// Public Function 1106
int GraphCore::GetMinimumVertexDegree() { return (m_i_MinimumVertexDegree); }

// Public Function 1107
double GraphCore::GetAverageVertexDegree() { return (m_d_AverageVertexDegree); }

// Public Function 1108
string GraphCore::GetInputFile() { return (m_s_InputFile); }

// Public Function 1109
void GraphCore::GetVertices(vector<int> &output) const { output = (m_vi_Vertices); }

// Public Function 1110
void GraphCore::GetEdges(vector<int> &output) const { output = (m_vi_Edges); }

// Public Function 1111
void GraphCore::GetValues(vector<double> &output) const { output = (m_vd_Values); }
...
bool GraphCore::AreD2Neighbor(int VertexIndex1, int VertexIndex2)
{
  ...
  set_intersection(D1_of_VertexIndex1.begin(), D1_of_VertexIndex1.end(), D1_of_VertexIndex2.begin(),
                   D1_of_VertexIndex2.end(), Intersect_set.begin());
  int size = Intersect_set.size();
  while (Intersect_set[size - 1] == -1 && size >= 1)
    size--;
...
bool GraphCore::operator==(const GraphCore &other) const
{
  ...
  if (m_vi_Vertices == other_Vertices && m_vi_Edges == other_Edges && m_vd_Values == other_Values)
    return true;
  else
    return false;
}
...
bool GraphCore::areEqual(const GraphCore &other, bool structureOnly) const
{
  ...
  { // structureOnly
    if (m_vi_Vertices == other_Vertices && m_vi_Edges == other_Edges)
      return true;
    else
      return false;
  }
