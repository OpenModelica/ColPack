...
void BipartiteGraphInputOutput::CalculateVertexDegrees()
{
  ...
  m_i_MaximumVertexDegree = m_i_MaximumLeftVertexDegree > m_i_MaximumRightVertexDegree ? m_i_MaximumLeftVertexDegree
                                                                                       : m_i_MaximumRightVertexDegree;
  m_i_MinimumVertexDegree = m_i_MinimumLeftVertexDegree < m_i_MinimumRightVertexDegree ? m_i_MinimumLeftVertexDegree
  ...
  m_d_AverageVertexDegree =
      (double)(i_TotalLeftVertexDegree + i_TotalRightVertexDegree) / (i_LeftVertexCount + i_RightVertexCount);

  return;
}

// Public Constructor 2251;3251
BipartiteGraphInputOutput::BipartiteGraphInputOutput() { Clear(); }
...
int BipartiteGraphInputOutput::ReadMatrixMarketBipartiteGraph(string s_InputFile)
{
  ...
  free(result);
  if (!(mm_is_coordinate(matcode) && (mm_is_symmetric(matcode) || mm_is_general(matcode)) &&
  ...
  { // entry_counter should be == num_of_entries
  ...
  cout << endl;
  cout << "[Left Vertices = " << i_LeftVertexCount << "; Right Vertices = " << i_RightVertexCount
       << "; Edges = " << i_EdgeCount / 2 << "]" << endl;
  cout << endl;
...
int BipartiteGraphInputOutput::ReadGenericMatrixBipartiteGraph(string s_InputFile)
{
  ...
  for (; rowCounter < row; rowCounter++)
  {
    ...
    if (colCounter != col)
    {
      ...
      cerr << "Input file\"" << s_InputFile << "\" has a wrong format. The number of entries in 1 column < # of columns"
           << endl;
      return _FALSE;
    }
  }
  ...
  {
    cerr << "WARNING: BipartiteGraphInputOutput::ReadGenericMatrixBipartiteGraph()" << endl;
    cout << "Input file\"" << s_InputFile
         << "\" has a wrong format. The number of rows is less than what it suppose to be" << endl;
    return _FALSE;
  }
  // put together the right vertices
  m_vi_RightVertices.push_back(m_vi_Edges.size());
...
{
  cout << endl;
  cout << "Bipartite Graph | " << m_s_InputFile << " | Maximum Row Vertex Degree | " << m_i_MaximumLeftVertexDegree
       << endl;
  cout << "Bipartite Graph | " << m_s_InputFile << " | Maximum Column Vertex Degree | " << m_i_MaximumRightVertexDegree
       << endl;
  cout << "Bipartite Graph | " << m_s_InputFile << " | Maximum Vertex Degree | " << m_i_MaximumVertexDegree << endl;
  cout << endl;
  cout << "Bipartite Graph | " << m_s_InputFile << " | Minimum Row Vertex Degree | " << m_i_MinimumLeftVertexDegree
       << endl;
  cout << "Bipartite Graph | " << m_s_InputFile << " | Minimum Column Vertex Degree | " << m_i_MinimumRightVertexDegree
       << endl;
  cout << "Bipartite Graph | " << m_s_InputFile << " | Minimum Vertex Degree | " << m_i_MinimumVertexDegree << endl;
  cout << endl;
  cout << "Bipartite Graph | " << m_s_InputFile << " | Average Row Vertex Degree | " << m_d_AverageLeftVertexDegree
       << endl;
  cout << "Bipartite Graph | " << m_s_InputFile << " | Average Column Vertex Degree | " << m_d_AverageRightVertexDegree
       << endl;
...
int BipartiteGraphInputOutput::BuildBPGraphFromCSRFormat(int *ip_RowIndex, int i_RowCount, int i_ColumnCount,
                                                         int *ip_ColumnIndex)
{
  int i;
  unsigned int j;
...
int BipartiteGraphInputOutput::BuildBPGraphFromADICFormat(std::list<std::set<int>> *lsi_SparsityPattern,
                                                          int i_ColumnCount)
{
...
int BipartiteGraphInputOutput::BuildBPGraphFromRowCompressedFormat(unsigned int **uip2_JacobianSparsityPattern,
                                                                   int i_RowCount, int i_ColumnCount)
{
  return RowCompressedFormat2BipartiteGraph(uip2_JacobianSparsityPattern, i_RowCount, i_ColumnCount);
}

int BipartiteGraphInputOutput::RowCompressedFormat2BipartiteGraph(unsigned int **uip2_JacobianSparsityPattern,
                                                                  int i_RowCount, int i_ColumnCount)
{
...
int BipartiteGraphInputOutput::BipartiteGraph2RowCompressedFormat(unsigned int ***uip3_JacobianSparsityPattern,
                                                                  ...
                                                                  unsigned int *uip1_ColumnCount)
{
  ...
  for (i = 0; i < i_RowCount; i++)
  {
    ...
    (*uip3_JacobianSparsityPattern)[i][0] =
        numOfNonZeros; // Populate the first entry, which contains the number of Non-Zeros
...
int BipartiteGraphInputOutput::ReadMMBipartiteGraphCpp11(string s_InputFile)
{
  ...
  m_d_AverageVertexDegree =
      (m_vi_LeftVertices.back() - m_vi_LeftVertices.front() + m_vi_RightVertices.back() - m_vi_RightVertices.front()) *
      1.0 / (row_count + col_count);
...
int BipartiteGraphInputOutput::ReadMMGeneralGraphIntoPothenBipartiteGraphCpp11(string s_InputFile)
{
  ...
  {
    printf(
        "Error, ReadMMGenearlGraphIntoPothenBipartiteGraphCpp11() tries to open \"%s\". But the file cannot be open.\n",
        s_InputFile.c_str());
    exit(1);
  }
  ...
  if (!(iss >> word) || word != "%%MatrixMarket" || !(iss >> word) || word != "matrix")
  {
    printf("Error,ReadMMGeneralGraphIntoPothenBipartiteGraphCpp11() tries to open \"%s\". But it is not MatrixMarket "
           "format\n",
           s_InputFile.c_str());
    exit(1);
  }
  if (!(iss >> word) || word != "coordinate")
  { // coordinate/array
    printf("Error, ReadMMGeneralGraphIntoPothenBipartiteGraphCpp11() tries to open \"%s\". But the graph is a complet "
           "graph.\n",
           s_InputFile.c_str());
    exit(1);
  }
  ...
  { // real/integer/complex/pattern
    printf("Warning, ReadMMGeneralGraphIntoPothenBipartiteGraphCpp11() tries to open \"%s\" and find it is complex "
           "value graph.\n",
           s_InputFile.c_str());
  }

  if (!(iss >> word) || word == "general")
  { // geneal/symmetric/skew-symmetric/Hermitian
    b_symmetric = false;
    printf("Warning! ReadMMGeneralGraphIntoPothenBipartiteGraphCpp11() expect to open '%s' as a symmtrix matrix with "
           "non-diagonal elements. But the graph is '%s' not 'symmetric'.\nThus the diagnal elements and upper "
           "triangular part will be removed.\n",
  ...
  if (!in)
  {
    printf("Error, ReadMMGeneralGraphIntoPothenBipartiteGraphCpp11() tries to open\"%s\". But cannot read dimension "
           "inforation.\n",
           s_InputFile.c_str());
    exit(1);
  }
  ...
  {
    printf("Error, ReadMMGeneralGraphIntoPothenBipartiteGraphCpp11() find graph %s has %d rows and %d columns, it is "
           "not simple graph.\n",
  ...
  while (in)
  {
    ...
    if (r <= c)
    {
      if (b_symmetric && r != c)
      {
        printf("Error! ReadMMGeneralGraphIntoPothenBipartiteGraphCpp11() find an and entry in upper triangular matrix "
               "\"%s\"\nRow,Col=%d,%d\n",
  ...
  if (entry_encounter != expect_entries)
  {
    printf("Error, ReadMMGeneralGraphIntoPothenBipartiteGraphCpp11() tries to read \"%s\". But only read %d entries "
           "(expect %d)\n",
           s_InputFile.c_str(), entry_encounter, expect_entries);
    exit(1);
  }
