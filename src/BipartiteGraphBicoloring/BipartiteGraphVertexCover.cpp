...
BipartiteGraphVertexCover::BipartiteGraphVertexCover() { Clear(); }
...
int BipartiteGraphVertexCover::CoverVertex()
{
  ...
  vector<int> vi_LeftVertexDegree, vi_CodeZeroLeftVertexDegree, vi_CodeOneLeftVertexDegree, vi_CodeTwoLeftVertexDegree,
      vi_CodeThreeLeftVertexDegree;

  vector<int> vi_RightVertexDegree, vi_CodeZeroRightVertexDegree, vi_CodeOneRightVertexDegree,
      vi_CodeTwoRightVertexDegree, vi_CodeThreeRightVertexDegree;
  ...
  for (i = 0; i < i_LeftVertexCount; i++)
  {
    ...
    {
      cout << STEP_UP(m_mimi2_VertexEdgeMap[i][m_vi_Edges[j]]) << "\t" << " : " << STEP_UP(i) << " - "
  ...
  for (i = 0; i < STEP_UP(i_HighestCodeZeroLeftVertexDegree); i++)
  {
    ...
    for (lit_ListIterator = vli_GroupedCodeZeroLeftVertexDegree[i].begin();
         lit_ListIterator != vli_GroupedCodeZeroLeftVertexDegree[i].end(); lit_ListIterator++)
    {
  ...
  for (i = 0; i < STEP_UP(i_HighestCodeZeroRightVertexDegree); i++)
  {
    ...
    for (lit_ListIterator = vli_GroupedCodeZeroRightVertexDegree[i].begin();
         lit_ListIterator != vli_GroupedCodeZeroRightVertexDegree[i].end(); lit_ListIterator++)
    {
      if (j == STEP_DOWN(i_CodeZeroDegreeVertexCount))
      {
        cout << STEP_UP(*lit_ListIterator) << " (" << i_CodeZeroDegreeVertexCount << ")";
  ...
  while (i_CodeZeroEdgeCount)
  {
    ...
    cout << "Candidate Left Vertex = " << STEP_UP(i_CandidateLeftVertex)
         << "; Candidate Right Vertex = " << STEP_UP(i_CandidateRightVertex) << endl;
    cout << endl;

#endif

    i_CodeZeroOneLeftVertexDegree =
        vi_CodeZeroLeftVertexDegree[i_CandidateLeftVertex] + vi_CodeOneLeftVertexDegree[i_CandidateLeftVertex];

    i_CodeZeroOneRightVertexDegree =
        vi_CodeZeroRightVertexDegree[i_CandidateRightVertex] + vi_CodeOneRightVertexDegree[i_CandidateRightVertex];

    i_QuotientOne = i_HighestCodeTwoLeftVertexDegree > i_CodeZeroOneLeftVertexDegree ? i_HighestCodeTwoLeftVertexDegree
                                                                                     : i_CodeZeroOneLeftVertexDegree;
    i_QuotientOne += i_HighestCodeThreeRightVertexDegree;

    i_QuotientTwo = i_HighestCodeThreeRightVertexDegree > i_CodeZeroOneRightVertexDegree
                        ? i_HighestCodeThreeRightVertexDegree
                        : i_CodeZeroOneRightVertexDegree;
    ...
    cout << "Selected Left Vertex = " << STEP_UP(i_CandidateLeftVertex)
         << "; Selected Right Vertex = " << STEP_UP(i_CandidateRightVertex) << endl;
    ...
    if (i_CandidateRightVertex == _UNKNOWN)
    {
      ...
      vli_GroupedCodeZeroLeftVertexDegree[vi_CodeZeroLeftVertexDegree[i_CandidateLeftVertex]].erase(
          vlit_CodeZeroLeftVertexLocation[i_CandidateLeftVertex]);
      ...
      for (i = m_vi_LeftVertices[i_CandidateLeftVertex]; i < m_vi_LeftVertices[STEP_UP(i_CandidateLeftVertex)]; i++)
      {
        ...
        if ((vi_EdgeCodes[i_PresentEdge] == _FALSE) || (vi_EdgeCodes[i_PresentEdge] == _TRUE))
        {
          if (vi_EdgeCodes[i_PresentEdge] == _FALSE)
          {
            ...
            {
              vli_GroupedCodeZeroRightVertexDegree[vi_CodeZeroRightVertexDegree[m_vi_Edges[i]]].erase(
                  vlit_CodeZeroRightVertexLocation[m_vi_Edges[i]]);
            }
          ...
          cout << "Edge " << STEP_UP(i_CandidateLeftVertex) << " - " << STEP_UP(m_vi_Edges[i]) << " ["
               << STEP_UP(i_PresentEdge) << "] : Code Changed From " << vi_EdgeCodes[i_PresentEdge] << " To 2" << endl;

#endif

          vi_EdgeCodes[i_PresentEdge] = 2;

          vi_CodeTwoLeftVertexDegree[i_CandidateLeftVertex] =
              STEP_UP(vi_CodeTwoLeftVertexDegree[i_CandidateLeftVertex]);
          ...
          for (j = m_vi_RightVertices[m_vi_Edges[i]]; j < m_vi_RightVertices[STEP_UP(m_vi_Edges[i])]; j++)
          {
            ...
            if (vi_EdgeCodes[i_NeighboringEdge] == _FALSE)
            {
              ...
              {
                vli_GroupedCodeZeroLeftVertexDegree[vi_CodeZeroLeftVertexDegree[m_vi_Edges[j]]].erase(
                    vlit_CodeZeroLeftVertexLocation[m_vi_Edges[j]]);
              }
              ...
              if (vi_CodeZeroLeftVertexDegree[m_vi_Edges[j]] > _UNKNOWN)
              {
                vli_GroupedCodeZeroLeftVertexDegree[vi_CodeZeroLeftVertexDegree[m_vi_Edges[j]]].push_front(
                    m_vi_Edges[j]);

                vlit_CodeZeroLeftVertexLocation[m_vi_Edges[j]] =
                    vli_GroupedCodeZeroLeftVertexDegree[vi_CodeZeroLeftVertexDegree[m_vi_Edges[j]]].begin();
              }
              ...
              if (vi_CodeZeroRightVertexDegree[m_vi_Edges[i]] > _UNKNOWN)
              {
                vli_GroupedCodeZeroRightVertexDegree[vi_CodeZeroRightVertexDegree[m_vi_Edges[i]]].push_front(
                    m_vi_Edges[i]);

                vlit_CodeZeroRightVertexLocation[m_vi_Edges[i]] =
                    vli_GroupedCodeZeroRightVertexDegree[vi_CodeZeroRightVertexDegree[m_vi_Edges[i]]].begin();
              }

#if DEBUG == 3355

              cout << "Edge " << STEP_UP(m_vi_Edges[j]) << " - " << STEP_UP(m_vi_Edges[i]) << " ["
                   << STEP_UP(i_NeighboringEdge) << "] : Code Changed From " << vi_EdgeCodes[i_NeighboringEdge]
                   << " To 1" << endl;
    ...
    else if (i_CandidateLeftVertex == _UNKNOWN)
    {
      m_vi_IncludedRightVertices[i_CandidateRightVertex] = _FALSE;

      vli_GroupedCodeZeroRightVertexDegree[vi_CodeZeroRightVertexDegree[i_CandidateRightVertex]].erase(
          vlit_CodeZeroRightVertexLocation[i_CandidateRightVertex]);
      ...
      for (i = m_vi_RightVertices[i_CandidateRightVertex]; i < m_vi_RightVertices[STEP_UP(i_CandidateRightVertex)]; i++)
      {
        ...
        if ((vi_EdgeCodes[i_PresentEdge] == _FALSE) || (vi_EdgeCodes[i_PresentEdge] == _TRUE))
        {
          if (vi_EdgeCodes[i_PresentEdge] == _FALSE)
          {
            ...
            {
              vli_GroupedCodeZeroLeftVertexDegree[vi_CodeZeroLeftVertexDegree[m_vi_Edges[i]]].erase(
                  vlit_CodeZeroLeftVertexLocation[m_vi_Edges[i]]);
            }
...
#if DEBUG == 3355
          cout << "Edge " << STEP_UP(m_vi_Edges[i]) << " - " << STEP_UP(i_CandidateRightVertex) << " ["
               << STEP_UP(i_PresentEdge) << "] : Code Changed From " << vi_EdgeCodes[i_PresentEdge] << " To 3" << endl;
#endif
          ...
          vi_CodeThreeRightVertexDegree[i_CandidateRightVertex] =
              STEP_UP(vi_CodeThreeRightVertexDegree[i_CandidateRightVertex]);
          ...
          for (j = m_vi_LeftVertices[m_vi_Edges[i]]; j < m_vi_LeftVertices[STEP_UP(m_vi_Edges[i])]; j++)
          {
            ...
            if (vi_EdgeCodes[i_NeighboringEdge] == _FALSE)
            {
              ...
              if (vi_CodeZeroLeftVertexDegree[m_vi_Edges[i]] > _UNKNOWN)
              {
                vli_GroupedCodeZeroLeftVertexDegree[vi_CodeZeroLeftVertexDegree[m_vi_Edges[i]]].push_front(
                    m_vi_Edges[i]);

                vlit_CodeZeroLeftVertexLocation[m_vi_Edges[i]] =
                    vli_GroupedCodeZeroLeftVertexDegree[vi_CodeZeroLeftVertexDegree[m_vi_Edges[i]]].begin();
              }

              if (vi_CodeZeroRightVertexDegree[m_vi_Edges[j]] > _UNKNOWN)
              {
                vli_GroupedCodeZeroRightVertexDegree[vi_CodeZeroRightVertexDegree[m_vi_Edges[j]]].erase(
                    vlit_CodeZeroRightVertexLocation[m_vi_Edges[j]]);
              }
              ...
              if (vi_CodeZeroRightVertexDegree[m_vi_Edges[j]] > _UNKNOWN)
              {
                vli_GroupedCodeZeroRightVertexDegree[vi_CodeZeroRightVertexDegree[m_vi_Edges[j]]].push_front(
                    m_vi_Edges[j]);

                vlit_CodeZeroRightVertexLocation[m_vi_Edges[j]] =
                    vli_GroupedCodeZeroRightVertexDegree[vi_CodeZeroRightVertexDegree[m_vi_Edges[j]]].begin();
              }

#if DEBUG == 3355

              cout << "Edge " << STEP_UP(m_vi_Edges[i]) << " - " << STEP_UP(m_vi_Edges[j]) << " ["
                   << STEP_UP(i_NeighboringEdge) << "] : Code Changed From " << vi_EdgeCodes[i_NeighboringEdge]
                   << " To 1" << endl;
  ...
  for (i = 0; i < i_LeftVertexCoverSize; i++)
  {
    ...
    i_VertexDegree =
        m_vi_LeftVertices[STEP_UP(m_vi_CoveredLeftVertices[i])] - m_vi_LeftVertices[m_vi_CoveredLeftVertices[i]];

    k = _FALSE;

    for (j = m_vi_LeftVertices[m_vi_CoveredLeftVertices[i]];
         j < m_vi_LeftVertices[STEP_UP(m_vi_CoveredLeftVertices[i])]; j++)
    {
  ...
  for (i = 0; i < i_RightVertexCoverSize; i++)
  {
    ...
    i_VertexDegree =
        m_vi_RightVertices[STEP_UP(m_vi_CoveredRightVertices[i])] - m_vi_RightVertices[m_vi_CoveredRightVertices[i]];

    k = _FALSE;

    for (j = m_vi_RightVertices[m_vi_CoveredRightVertices[i]];
         j < m_vi_RightVertices[STEP_UP(m_vi_CoveredRightVertices[i])]; j++)
    {
      if (k == STEP_DOWN(i_VertexDegree))
      {
  ...
  cout << endl;
  cout << "[Left Vertex Cover Size = " << i_LeftVertexCoverSize
       << "; Right Vertex Cover Size = " << i_RightVertexCoverSize << "; Edges Covered = " << i_CoveredEdgeCount << "]"
       << endl;
  cout << endl;
...
int BipartiteGraphVertexCover::CoverMinimalVertex()
{
    ...
    if (i_SelectedVertex < i_LeftVertexCount)
    {
      ...
      for (i = m_vi_LeftVertices[i_PresentVertex]; i < m_vi_LeftVertices[STEP_UP(i_PresentVertex)]; i++)
      {
        ...
        for (j = m_vi_RightVertices[i_NeighboringVertex]; j < m_vi_RightVertices[STEP_UP(i_NeighboringVertex)]; j++)
        {
          ...
          vli_GroupedVertexDegree[vi_VertexDegree[i_SecondNeighboringVertex]].erase(
              vlit_VertexLocation[i_SecondNeighboringVertex]);
          ...
          vlit_VertexLocation[i_SecondNeighboringVertex] =
              vli_GroupedVertexDegree[vi_VertexDegree[i_SecondNeighboringVertex]].begin();
        ...
        vli_GroupedVertexDegree[vi_VertexDegree[i_NeighboringVertex + i_LeftVertexCount]].erase(
            vlit_VertexLocation[i_NeighboringVertex + i_LeftVertexCount]);
    ...
    else
    {
      ...
      for (i = m_vi_RightVertices[i_PresentVertex]; i < m_vi_RightVertices[STEP_UP(i_PresentVertex)]; i++)
      {
        ...
        for (j = m_vi_RightVertices[i_NeighboringVertex]; j < m_vi_RightVertices[STEP_UP(i_NeighboringVertex)]; j++)
        {
          ...
          vli_GroupedVertexDegree[vi_VertexDegree[i_SecondNeighboringVertex + i_LeftVertexCount]].erase(
              vlit_VertexLocation[i_SecondNeighboringVertex + i_LeftVertexCount]);
          ...
          vli_GroupedVertexDegree[vi_VertexDegree[i_SecondNeighboringVertex + i_LeftVertexCount]].push_front(
              i_SecondNeighboringVertex + i_LeftVertexCount);

          vlit_VertexLocation[i_SecondNeighboringVertex + i_LeftVertexCount] =
              vli_GroupedVertexDegree[vi_VertexDegree[i_SecondNeighboringVertex + i_LeftVertexCount]].begin();
  ...
  cout << endl;
  cout << "[Vertex Covers Size = " << i_LeftVertexCoverSize + i_RightVertexCoverSize
       << "; Independent Set Size = " << i_IndependentSetSize
       << "; Vertex Count = " << i_LeftVertexCount + i_RightVertexCount << "]" << endl;
  cout << "[Left Vertex Cover Size = " << i_LeftVertexCoverSize
       << "; Right Vertex Cover Size = " << i_RightVertexCoverSize << "; Edges Covered = " << i_CoveredEdgeCount << "/"
       << i_EdgeCount << "]" << endl;
  cout << endl;
...
void BipartiteGraphVertexCover::GetIncludedLeftVertices(vector<int> &output) { output = (m_vi_IncludedLeftVertices); }

// Public Function 3360
void BipartiteGraphVertexCover::GetIncludedRightVertices(vector<int> &output) { output = (m_vi_IncludedRightVertices); }

// Public Function 3361
void BipartiteGraphVertexCover::GetCoveredLeftVertices(vector<int> &output) { output = (m_vi_CoveredLeftVertices); }

// Public Function 3362
void BipartiteGraphVertexCover::GetCoveredRightVertices(vector<int> &output) { output = (m_vi_CoveredRightVertices); }
