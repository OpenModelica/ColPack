...
int BipartiteGraphBicoloring::CalculateVertexColorClasses()
{
  ...
  for (int i = 0; i < m_i_LeftVertexColorCount; i++)
  {
    ...
    else if (m_i_SmallestLeftVertexColorClassSize > m_vi_LeftVertexColorFrequency[i])
    {
      m_i_SmallestLeftVertexColorClass = i;
  ...
  for (int i = 0; i < m_i_RightVertexColorCount; i++)
  {
    ...
    else if (m_i_SmallestRightVertexColorClassSize > m_vi_RightVertexColorFrequency[i])
    {
      m_i_SmallestRightVertexColorClass = i;
  ...
  m_i_LargestVertexColorClassSize = m_i_LargestLeftVertexColorClassSize > m_i_LargestRightVertexColorClassSize
                                        ? m_i_LargestLeftVertexColorClassSize
                                        : m_i_LargestRightVertexColorClassSize;
  m_i_LargestVertexColorClass = m_i_LargestVertexColorClassSize == m_i_LargestLeftVertexColorClassSize
                                    ? m_i_LargestLeftVertexColorClass
                                    : m_i_LargestRightVertexColorClass;

  m_i_SmallestVertexColorClassSize = m_i_SmallestLeftVertexColorClassSize < m_i_SmallestRightVertexColorClassSize
                                         ? m_i_SmallestLeftVertexColorClassSize
                                         : m_i_SmallestRightVertexColorClassSize;
  m_i_SmallestVertexColorClass = m_i_SmallestVertexColorClassSize == m_i_SmallestLeftVertexColorClassSize
                                     ? m_i_SmallestLeftVertexColorClass
                                     : m_i_SmallestRightVertexColorClass;
...
int BipartiteGraphBicoloring::FixMinimalCoverStarBicoloring()
{
  ...
  for (i = 0; i < i_RightVertexCount; i++)
  {
    if (vi_VertexColors[m_vi_RightVertexColors[i]] == _TRUE)
    {
      ...
      cout << "Color Violation " << i_ColorViolationCount << " | Right Vertex " << STEP_UP(i) << " | Conflicting Color "
           << m_vi_RightVertexColors[i] << endl;
      ...
      cout << "Fixed Color Violation " << i_ColorViolationCount << " | Right Vertex " << STEP_UP(i)
           << " | Changed Right Vertex Color " << m_vi_RightVertexColors[i] << endl;
        ...
        if (i_ThirdColor == i_FirstColor)
        {
          for (l = m_vi_LeftVertices[m_vi_Edges[k]]; l < m_vi_LeftVertices[STEP_UP(m_vi_Edges[k])]; l++)
          {
            ...
            if (i_FourthColor == i_SecondColor)
            {
              ...
              cout << "Path Violation " << i_PathViolationCount << " | " << STEP_UP(i) << " [" << i_FirstColor << "] - "
                   << STEP_UP(m_vi_Edges[j]) << " [" << i_SecondColor << "] - " << STEP_UP(m_vi_Edges[k]) << " ["
                   << i_ThirdColor << "] - " << STEP_UP(m_vi_Edges[l]) << " [" << i_FourthColor << "]" << endl;
              ...
              cout << "Fixed Path Violation " << i_PathViolationCount << " | " << STEP_UP(i) << " [" << i_FirstColor
                   << "] - " << STEP_UP(m_vi_Edges[j]) << " [" << i_SecondColor << "] - " << STEP_UP(m_vi_Edges[k])
                   << " [" << i_ThirdColor << "] - " << STEP_UP(m_vi_Edges[l]) << " ["
                   << m_vi_RightVertexColors[m_vi_Edges[l]] << "]" << endl;
...
int BipartiteGraphBicoloring::MinimalCoveringRowMajorStarBicoloring()
{
  ...
  cout << "DEBUG 3556 | Left Star Bicoloring | Left Vertex Cover Size = " << m_vi_CoveredLeftVertices.size()
       << "; Right Vertex Cover Size = " << m_vi_CoveredRightVertices.size() << endl;
        ...
        else
        {
          ...
          for (k = m_vi_LeftVertices[i_NeighboringVertex]; k < m_vi_LeftVertices[STEP_UP(i_NeighboringVertex)]; k++)
          {
            ...
            if (vi_RightStarHubMap
                    [vi_EdgeStarMap[m_mimi2_VertexEdgeMap[i_NeighboringVertex][i_SecondNeighboringVertex]]] ==
...
int BipartiteGraphBicoloring::MinimalCoveringColumnMajorStarBicoloring()
{
  ...
  cout << "DEBUG 3557 | Right Star Bicoloring | Left Vertex Cover Size = " << m_vi_CoveredLeftVertices.size()
       << "; Right Vertex Cover Size = " << m_vi_CoveredRightVertices.size() << endl;
      ...
      else
      {
        ...
        else
        {
          ...
          for (k = m_vi_RightVertices[i_NeighboringVertex]; k < m_vi_RightVertices[STEP_UP(i_NeighboringVertex)]; k++)
          {
            ...
            if (vi_LeftStarHubMap
                    [vi_EdgeStarMap[m_mimi2_VertexEdgeMap[i_SecondNeighboringVertex][i_NeighboringVertex]]] ==
                i_SecondNeighboringVertex)
            {
              vi_CandidateColors[m_vi_LeftVertexColors[i_SecondNeighboringVertex]] = i_PresentVertex;
            }
...
int BipartiteGraphBicoloring::ExplicitCoveringModifiedStarBicoloring()
{
  ...
  for (i = 0; i < i_LeftVertexCount; i++)
  {
    for (j = m_vi_LeftVertices[i]; j < m_vi_LeftVertices[STEP_UP(i)]; j++)
    {
      ...
      else if (vi_EdgeCodes[i_EdgeID] == 1)
      {
        i_EdgeCodeOne++;
      }
      else if (vi_EdgeCodes[i_EdgeID] == 2)
      {
        i_EdgeCodeTwo++;
      }
      else if (vi_EdgeCodes[i_EdgeID] == 3)
      {
        i_EdgeCodeThree++;
      }
    }
  ...
  cout << endl;
  cout << "Code Zero Edges = " << i_EdgeCodeZero << "; Code One Edges = " << i_EdgeCodeOne
       << "; Code Two Edges = " << i_EdgeCodeTwo << "; Code Three Edges = " << i_EdgeCodeThree << endl;
  cout << endl;
  ...
  cout << "DEBUG 3558 | Star Bicoloring | Left Vertex Cover Size = " << m_vi_CoveredLeftVertices.size()
       << "; Right Vertex Cover Size = " << m_vi_CoveredRightVertices.size() << endl;
      ...
      for (j = m_vi_LeftVertices[i_PresentVertex]; j < m_vi_LeftVertices[STEP_UP(i_PresentVertex)]; j++)
      {
        ...
        for (k = m_vi_RightVertices[i_NeighboringVertex]; k < m_vi_RightVertices[STEP_UP(i_NeighboringVertex)]; k++)
        {
          ...
          if (vi_EdgeCodes[i_NeighboringEdgeID] != 2)
          {
            ...
            {
              for (l = m_vi_LeftVertices[i_SecondNeighboringVertex];
                   l < m_vi_LeftVertices[STEP_UP(i_SecondNeighboringVertex)]; l++)
              {
      ...
      for (j = m_vi_RightVertices[i_PresentVertex]; j < m_vi_RightVertices[STEP_UP(i_PresentVertex)]; j++)
      {
        ...
        for (k = m_vi_LeftVertices[i_NeighboringVertex]; k < m_vi_LeftVertices[STEP_UP(i_NeighboringVertex)]; k++)
        {
          ...
          if (vi_EdgeCodes[i_NeighboringEdgeID] != 3)
          {
            ...
            {
              for (l = m_vi_RightVertices[i_SecondNeighboringVertex];
                   l < m_vi_RightVertices[STEP_UP(i_SecondNeighboringVertex)]; l++)
              {
                i_ThirdNeighboringVertex = m_vi_Edges[l];
...
int BipartiteGraphBicoloring::ExplicitCoveringStarBicoloring()
{
  ...
  cout << "DEBUG 3559 | Combined Star Bicoloring | Left Vertex Cover Size = " << m_vi_CoveredLeftVertices.size()
       << "; Right Vertex Cover Size = " << m_vi_CoveredRightVertices.size() << endl;
  ...
  for (i = 0; i < i_LeftVertexCount; i++)
  {
    cout << STEP_UP(i) << "\t" << " : " << m_vi_LeftVertexColors[i] << " [" << m_vi_IncludedLeftVertices[i] << "]"
         << endl;
  }
  ...
  for (i = 0; i < i_RightVertexCount; i++)
  {
    cout << STEP_UP(i) << "\t" << " : " << m_vi_RightVertexColors[i] << " [" << m_vi_IncludedRightVertices[i] << "]"
         << endl;
  }
...
int BipartiteGraphBicoloring::MinimalCoveringStarBicoloring()
{
  ...
  cout << "DEBUG 3560 | Combined Star Bicoloring | Left Vertex Cover Size = " << m_vi_CoveredLeftVertices.size()
       << "; Right Vertex Cover Size = " << m_vi_CoveredRightVertices.size() << endl;
          ...
          if (i_FirstNeighborOne == i_PresentVertex)
          {
            if (vi_LeftTreated[i_FirstNeighborTwo] != i_PresentVertex)
            {
              for (k = m_vi_RightVertices[i_FirstNeighborTwo]; k < m_vi_RightVertices[STEP_UP(i_FirstNeighborTwo)]; k++)
              {
                ...
                cout << "DEBUG 3561 | Star Bicoloring | Visited Same Color Neighbor | Preventing Color "
                     << m_vi_LeftVertexColors[i_SecondNeighboringVertex] << " of Left Neighboring Vertex "
                     << STEP_UP(i_SecondNeighboringVertex) << " coming from Right Neighboring Vertex "
                     << STEP_UP(i_FirstNeighborTwo) << endl;
            ...
            for (k = m_vi_RightVertices[m_vi_Edges[j]]; k < m_vi_RightVertices[STEP_UP(m_vi_Edges[j])]; k++)
            {
              ...
              cout << "DEBUG 3561 | Star Bicoloring | Restricted Same Color Neighbor | Preventing Color "
                   << m_vi_LeftVertexColors[i_SecondNeighboringVertex] << " of Left Neighboring Vertex "
                   << STEP_UP(i_SecondNeighboringVertex) << " coming from Right Neighboring Vertex "
          ...
          else
          {
            ...
            for (k = m_vi_RightVertices[i_NeighboringVertex]; k < m_vi_RightVertices[STEP_UP(i_NeighboringVertex)]; k++)
            {
                  ...
                  i_SecondNeighboringVertex)
              {
                ...
                cout << "DEBUG 3561 | Star Bicoloring | Restricted Hub Color Neighbor | Preventing Color "
                     << m_vi_LeftVertexColors[i_SecondNeighboringVertex] << " of Left Neighboring Vertex "
                     << STEP_UP(i_SecondNeighboringVertex) << " coming from Right Neighboring Vertex "
                     << STEP_UP(i_NeighboringVertex) << endl;
          ...
          if (i_FirstNeighborOne == i_PresentVertex)
          {
            if (vi_RightTreated[i_FirstNeighborTwo] != i_PresentVertex)
            {
              for (k = m_vi_LeftVertices[i_FirstNeighborTwo]; k < m_vi_LeftVertices[STEP_UP(i_FirstNeighborTwo)]; k++)
              {
                ...
                cout << "DEBUG 3561 | Star Bicoloring | Visited Same Color Neighbor | Preventing Color "
                     << m_vi_RightVertexColors[i_SecondNeighboringVertex] << " of Right Neighboring Vertex "
                     << STEP_UP(i_SecondNeighboringVertex) << " coming from Left Neighboring Vertex "
                     << STEP_UP(i_FirstNeighborTwo) << endl;
          ...
          else
          {
            ...
            for (k = m_vi_LeftVertices[i_NeighboringVertex]; k < m_vi_LeftVertices[STEP_UP(i_NeighboringVertex)]; k++)
            {
                  ...
                  i_SecondNeighboringVertex)
              {
                ...
                cout << "DEBUG 3561 | Star Bicoloring | Restricted Hub Color Neighbor | Preventing Color "
                     << m_vi_RightVertexColors[i_SecondNeighboringVertex] << " of Neighboring Vertex "
                     << STEP_UP(i_SecondNeighboringVertex) << " coming from Left Neighboring Vertex "
                     << STEP_UP(i_NeighboringVertex) << endl;
          ...
          if (i_FirstNeighborOne == i_PresentVertex)
          {
            if (vi_LeftTreated[i_FirstNeighborTwo] != i_PresentVertex)
            {
              for (k = m_vi_RightVertices[i_FirstNeighborTwo]; k < m_vi_RightVertices[STEP_UP(i_FirstNeighborTwo)]; k++)
              {
                ...
                cout << "DEBUG 3562 | Star Bicoloring | Visited Same Color Neighbor | Preventing Color "
                     << m_vi_LeftVertexColors[i_SecondNeighboringVertex] << " of Left Neighboring Vertex "
                     << STEP_UP(i_SecondNeighboringVertex) << " coming from Right Neighboring Vertex "
                     << STEP_UP(i_FirstNeighborTwo) << endl;
            ...
            for (k = m_vi_RightVertices[m_vi_Edges[j]]; k < m_vi_RightVertices[STEP_UP(m_vi_Edges[j])]; k++)
            {
              ...
              cout << "DEBUG 3562 | Star Bicoloring | Restricted Same Color Neighbor | Preventing Color "
                   << m_vi_LeftVertexColors[i_SecondNeighboringVertex] << " of Left Neighboring Vertex "
                   << STEP_UP(i_SecondNeighboringVertex) << " coming from Right Neighboring Vertex "
                   << STEP_UP(m_vi_Edges[j]) << endl;
          ...
          else
          {
            ...
            for (k = m_vi_RightVertices[i_NeighboringVertex]; k < m_vi_RightVertices[STEP_UP(i_NeighboringVertex)]; k++)
            {
                  ...
                  i_SecondNeighboringVertex)
              {
                ...
                cout << "DEBUG 3562 | Star Bicoloring | Restricted Hub Color Neighbor | Preventing Color "
                     << m_vi_LeftVertexColors[i_SecondNeighboringVertex] << " of Left Neighboring Vertex "
                     << STEP_UP(i_SecondNeighboringVertex) << " coming from Right Neighboring Vertex "
                     << STEP_UP(i_NeighboringVertex) << endl;
  ...
  for (i = 0; i < i_RightVertexCount; i++)
  {
    if (m_vi_RightVertexColors[i] == _UNKNOWN)
    {
      m_vi_RightVertexColors[i] =
          m_i_VertexColorCount; // m_i_VertexColorCount == (i_LeftVertexCount +  i_RightVertexCount + 1)
          ...
          if (i_FirstNeighborOne == i_PresentVertex)
          {
            if (vi_LeftTreated[i_FirstNeighborTwo] != i_PresentVertex)
            {
              for (k = m_vi_RightVertices[i_FirstNeighborTwo]; k < m_vi_RightVertices[STEP_UP(i_FirstNeighborTwo)]; k++)
              {
                ...
                cout << "DEBUG 3563 | Star Bicoloring | Visited Same Color Neighbor | Preventing Color "
                     << m_vi_LeftVertexColors[i_SecondNeighboringVertex] << " of Left Neighboring Vertex "
                     << STEP_UP(i_SecondNeighboringVertex) << " coming from Right Neighboring Vertex "
                     << STEP_UP(i_FirstNeighborTwo) << endl;
            ...
            for (k = m_vi_RightVertices[m_vi_Edges[j]]; k < m_vi_RightVertices[STEP_UP(m_vi_Edges[j])]; k++)
            {
              ...
              cout << "DEBUG 3563 | Star Bicoloring | Restricted Same Color Neighbor | Preventing Color "
                   << m_vi_LeftVertexColors[i_SecondNeighboringVertex] << " of Left Neighboring Vertex "
                   << STEP_UP(i_SecondNeighboringVertex) << " coming from Right Neighboring Vertex "
                   << STEP_UP(m_vi_Edges[j]) << endl;
          ...
          else
          {
            ...
            for (k = m_vi_RightVertices[i_NeighboringVertex]; k < m_vi_RightVertices[STEP_UP(i_NeighboringVertex)]; k++)
            {
                  ...
                  i_SecondNeighboringVertex)
              {
                ...
                cout << "DEBUG 3563 | Star Bicoloring | Restricted Hub Color Neighbor | Preventing Color "
                     << m_vi_LeftVertexColors[i_SecondNeighboringVertex] << " of Left Neighboring Vertex "
                     << STEP_UP(i_SecondNeighboringVertex) << " coming from Right Neighboring Vertex "
                     << STEP_UP(i_NeighboringVertex) << endl;
  ...
  for (i = 0; i < i_OrderedVertexCount; i++)
  {
    ...
    if (m_vi_OrderedVertices[i] < i_LeftVertexCount)
    {
      ...
      if (_FOUND == _FALSE)
      {
        ...
        cout << "DEBUG 3564 | Greedy Star Bicoloring | Ignored Present Left Vertex | " << STEP_UP(i_PresentVertex)
             << endl;
#endif
    ...
    else
    {
      ...
      if (_FOUND == _FALSE)
      {
        ...
        cout << "DEBUG 3564 | Greedy Star Bicoloring | Ignored Present Right Vertex | " << STEP_UP(i_PresentVertex)
             << endl;
#endif
...
int BipartiteGraphBicoloring::GetLeftVertexColorCount() { return (m_i_LeftVertexColorCount); }

// Public Function 3569
int BipartiteGraphBicoloring::GetRightVertexColorCount() { return (m_i_RightVertexColorCount); }

// Public Function 3570
int BipartiteGraphBicoloring::GetVertexColorCount() { return (m_i_VertexColorCount); }

// Public Function 3571
int BipartiteGraphBicoloring::GetViolationCount() { return (m_i_ViolationCount); }

int BipartiteGraphBicoloring::GetRightVertexDefaultColor() { return (i_RightVertexDefaultColor); }

string BipartiteGraphBicoloring::GetVertexColoringVariant() { return GetVertexBicoloringVariant(); }
...
string BipartiteGraphBicoloring::GetVertexBicoloringVariant()
{
  ...
  else if (m_s_VertexColoringVariant.compare("MINIMAL_COVER_COLUMN_STAR") == 0)
  {
    return ("Minimal Cover Column Star");
  }
  else if (m_s_VertexColoringVariant.compare("EXPLICIT_COVER_MODIFIED_STAR") == 0)
  {
    return ("Explicit Cover Modified Star");
  }
  else if (m_s_VertexColoringVariant.compare("EXPLICIT_COVER_STAR") == 0)
  {
    return ("Explicit Cover Star");
  }
  else if (m_s_VertexColoringVariant.compare("MINIMAL_COVER_STAR") == 0)
  {
    return ("Minimal Cover Star");
  }
  else if (m_s_VertexColoringVariant.compare("IMPLICIT_COVER_CONSERVATIVE_STAR") == 0)
  {
    return ("Implicit Cover Conservative Star");
  }
  else if (m_s_VertexColoringVariant.compare("IMPLICIT_COVER_STAR") == 0)
  {
    return ("Implicit Cover Star");
  }
  else if (m_s_VertexColoringVariant.compare("IMPLICIT_COVER_RESTRICTED_STAR") == 0)
  {
    return ("Implicit Cover Restricted Star");
  }
  else if (m_s_VertexColoringVariant.compare("IMPLICIT_COVER_GREEDY_STAR") == 0)
  {
    return ("Implicit Cover Greedy Star");
  }
  else if (m_s_VertexColoringVariant.compare("IMPLICIT_COVER_ACYCLIC") == 0)
  {
...
void BipartiteGraphBicoloring::GetLeftVertexColors(vector<int> &output) { output = m_vi_LeftVertexColors; }

// Public Function 3574
void BipartiteGraphBicoloring::GetRightVertexColors(vector<int> &output) { output = m_vi_RightVertexColors; }
...
void BipartiteGraphBicoloring::GetRightVertexColors_Transformed(vector<int> &output)
{
  ...
  {
    output[i] -= rowCount;
    if (output[i] == columnCount + 1)
      output[i] = 0; // color 0, the rows with this color should be ignored.
  }
}
...
void BipartiteGraphBicoloring::PrintVertexBicolorClasses()
{
  ...
  {
    cout << endl;
    cout << "Vertex Bicolor Classes | " << m_s_VertexColoringVariant << " Coloring | " << m_s_VertexOrderingVariant
         << " Ordering | " << m_s_InputFile << " | Vertex Bicolors Not Set" << endl;
    cout << endl;
  ...
  cout << endl;
  cout << "Row Color Classes | " << m_s_VertexColoringVariant << " Coloring | " << m_s_VertexOrderingVariant
  ...
  cout << endl;
  cout << "[Largest Row Color Class : " << STEP_UP(m_i_LargestLeftVertexColorClass)
       << "; Largest Row Color Class Size : " << m_i_LargestLeftVertexColorClassSize << "]" << endl;
  cout << "[Smallest Row Color Class : " << STEP_UP(m_i_SmallestLeftVertexColorClass)
       << "; Smallest Row Color Class Size : " << m_i_SmallestLeftVertexColorClassSize << "]" << endl;
  cout << "[Average Row Color Class Size : " << m_d_AverageLeftVertexColorClassSize << "]" << endl;
  cout << endl;

  cout << endl;
  cout << "Column Color Classes | " << m_s_VertexColoringVariant << " Coloring | " << m_s_VertexOrderingVariant
       << " Ordering | " << m_s_InputFile << endl;
  cout << endl;
  ...
  cout << endl;
  cout << "[Largest Column Color Class : " << STEP_UP(m_i_LargestRightVertexColorClass)
       << "; Largest Column Color Class Size : " << m_i_LargestRightVertexColorClassSize << "]" << endl;
  cout << "[Smallest Column Color Class : " << STEP_UP(m_i_SmallestRightVertexColorClass)
       << "; Smallest Column Color Class Size : " << m_i_SmallestRightVertexColorClassSize << "]" << endl;
  cout << "[Average Column Color Class Size : " << m_d_AverageRightVertexColorClassSize << "]" << endl;
  cout << endl;

  cout << endl;
  cout << "[Largest Vertex Color Class : " << STEP_UP(m_i_LargestVertexColorClass)
       << "; Largest Vertex Color Class Size : " << m_i_LargestVertexColorClassSize << "]" << endl;
  cout << "[Smallest Vertex Color Class : " << STEP_UP(m_i_SmallestVertexColorClass)
       << "; Smallest Vertex Color Class Size : " << m_i_SmallestVertexColorClassSize << "]" << endl;
...
void BipartiteGraphBicoloring::PrintVertexBicolors()
{
  ...
  cout << endl;
  cout << GetVertexBicoloringVariant() << " Bicoloring | " << GetVertexOrderingVariant()
       << " Ordering | Row Vertex Colors | " << s_InputFile << endl;
  cout << endl;
  ...
  cout << endl;
  cout << GetVertexBicoloringVariant() << " Bicoloring | " << GetVertexOrderingVariant()
       << " Ordering | Column Vertex Colors | " << s_InputFile << endl;
  cout << endl;
  ...
  cout << endl;
  cout << "[Total Vertex Colors = " << m_i_VertexColorCount << ", Violation Count = " << m_i_ViolationCount << "]"
       << endl;
  cout << endl;
...
void BipartiteGraphBicoloring::PrintVertexBicoloringMetrics()
{
  ...
  cout << endl;
  cout << GetVertexBicoloringVariant() << " Bicoloring | " << GetVertexOrderingVariant() << " Ordering | "
       << s_InputFile << endl;
  cout << endl;

  cout << endl;
  cout << "[Total Colors = " << m_i_VertexColorCount << "; Violation Count = " << m_i_ViolationCount << "]" << endl;
  cout << "[Row Vertex Count = " << STEP_DOWN(m_vi_LeftVertices.size())
       << "; Column Vertex Count = " << STEP_DOWN(m_vi_RightVertices.size()) << endl;
  cout << "[Ordering Time = " << m_d_OrderingTime << "; Covering Time = " << m_d_CoveringTime
...
double **BipartiteGraphBicoloring::GetLeftSeedMatrix_unmanaged(int *ip1_SeedRowCount, int *ip1_SeedColumnCount)
{
  ...
  if (i_LeftVertexDefaultColor == 1)
    i_num_of_colors--; // color ID 0 is used, ignore it
  (*ip1_SeedRowCount) = i_num_of_colors;
  (*ip1_SeedColumnCount) = i_size;
  if ((*ip1_SeedRowCount) == 0 || (*ip1_SeedColumnCount) == 0)
    return NULL;
  ...
  for (int i = 0; i < (*ip1_SeedRowCount); i++)
  {
    ...
    for (int j = 0; j < (*ip1_SeedColumnCount); j++)
      Seed[i][j] = 0.;
  }
  ...
  for (int i = 0; i < (*ip1_SeedColumnCount); i++)
  {
    ...
    {
      printf("**WARNING: Out of bound: Seed[%d >= %d][%d] = 1. \n", m_vi_LeftVertexColors[i] - 1,
             (*ip1_SeedColumnCount), i);
    }
#endif
...
double **BipartiteGraphBicoloring::GetRightSeedMatrix_unmanaged(int *ip1_SeedRowCount, int *ip1_SeedColumnCount)
{
  ...
  if (i_RightVertexDefaultColor == 1)
    i_num_of_colors--; // color ID 0 is used, ignore it
  ...
  for (int i = 0; i < (*ip1_SeedRowCount); i++)
  {
    ...
    {
      printf("**WARNING: Out of bound: Seed[%d][%d >= %d] = 1. \n", i, RightVertexColors_Transformed[i] - 1,
             (*ip1_SeedRowCount));
    }
#endif
    if (RightVertexColors_Transformed[i] != 0)
    { // ignore color 0
...
double BipartiteGraphBicoloring::GetVertexColoringTime() { return m_d_ColoringTime; }

void BipartiteGraphBicoloring::GetSeedMatrix(double ***dp3_LeftSeed, int *ip1_LeftSeedRowCount,
                                             int *ip1_LeftSeedColumnCount, double ***dp3_RightSeed,
                                             int *ip1_RightSeedRowCount, int *ip1_RightSeedColumnCount)
{
...
void BipartiteGraphBicoloring::GetSeedMatrix_unmanaged(double ***dp3_LeftSeed, int *ip1_LeftSeedRowCount,
                                                       int *ip1_LeftSeedColumnCount, double ***dp3_RightSeed,
                                                       int *ip1_RightSeedRowCount, int *ip1_RightSeedColumnCount)
{
