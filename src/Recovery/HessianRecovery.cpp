...
int HessianRecovery::DirectRecover_RowCompressedFormat_usermem(GraphColoringInterface *g, double **dp2_CompressedMatrix,
                                                               ...
                                                               double ***dp3_HessianValue)
{
  ...
  int **colorStatistic =
      new int *[rowCount]; // color statistic for each row. For example, colorStatistic[0] is color statistic for row 0
  ...
  for (unsigned int i = 0; i < (unsigned int)rowCount; i++)
  {
    ...
    for (unsigned int j = 1; j <= numOfNonZeros; j++)
    {
      ...
      { // the non-zero is in the diagonal of the matrix
      ...
      { // i != uip2_HessianSparsityPattern[i][j] // the non-zero is NOT in the diagonal of the matrix
...
int HessianRecovery::DirectRecover_RowCompressedFormat_unmanaged(GraphColoringInterface *g,
                                                                 ...
                                                                 double ***dp3_HessianValue)
{
  ...
  *dp3_HessianValue = (double **)malloc(rowCount * sizeof(double *));
  for (unsigned int i = 0; i < (unsigned int)rowCount; i++)
  {
    ...
    for (unsigned int j = 1; j <= numOfNonZeros; j++)
      (*dp3_HessianValue)[i][j] = 0.; // initialize value of other entries
  }

  return DirectRecover_RowCompressedFormat_usermem(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern,
                                                   dp3_HessianValue);
}

int HessianRecovery::DirectRecover_RowCompressedFormat(GraphColoringInterface *g, double **dp2_CompressedMatrix,
                                                       unsigned int **uip2_HessianSparsityPattern,
                                                       double ***dp3_HessianValue)
{

  int returnValue = DirectRecover_RowCompressedFormat_unmanaged(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern,
                                                                dp3_HessianValue);
...
int HessianRecovery::DirectRecover_SparseSolversFormat_usermem(GraphColoringInterface *g, double **dp2_CompressedMatrix,
                                                               unsigned int **uip2_HessianSparsityPattern,
                                                               ...
                                                               unsigned int numOfNonZerosInHessianValue)
{
  ...
  for (unsigned int i = 0; i < (unsigned int)rowCount; i++)
  {
    ...
    for (unsigned int j = 1; j <= numOfNonZeros; j++)
    {
      ...
      else if (i == uip2_HessianSparsityPattern[i][j])
      { // the non-zero is in the diagonal of the matrix
      ...
      { // i != uip2_HessianSparsityPattern[i][j] // the non-zero is NOT in the diagonal of the matrix
        if (colorStatistic[i][vi_VertexColors[uip2_HessianSparsityPattern[i][j]]] == 1)
        {
          (*dp2_HessianValue)[(*ip2_RowIndex)[i] + j - offset - 1] =
              dp2_CompressedMatrix[i][vi_VertexColors[uip2_HessianSparsityPattern[i][j]]];
        ...
        {
          (*dp2_HessianValue)[(*ip2_RowIndex)[i] + j - offset - 1] =
              dp2_CompressedMatrix[uip2_HessianSparsityPattern[i][j]][vi_VertexColors[i]];
...
int HessianRecovery::DirectRecover_SparseSolversFormat_unmanaged(
    GraphColoringInterface *g, double **dp2_CompressedMatrix, unsigned int **uip2_HessianSparsityPattern,
    unsigned int **ip2_RowIndex, unsigned int **ip2_ColumnIndex, double **dp2_HessianValue,
    unsigned int numOfNonZerosInHessianValue)
{
  ...
  {
    numOfNonZerosInHessianValue = ConvertRowCompressedFormat2SparseSolversFormat_StructureOnly(
        uip2_HessianSparsityPattern, rowCount, ip2_RowIndex, ip2_ColumnIndex);
  ...
  (*dp2_HessianValue) =
      (double *)malloc(numOfNonZerosInHessianValue * sizeof(double)); // allocate memory for *dp2_JacobianValue.
  for (unsigned int i = 0; i < numOfNonZerosInHessianValue; i++)
    (*dp2_HessianValue)[i] = 0.; // initialize value of other entries

  int returnValue =
      DirectRecover_SparseSolversFormat_usermem(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern, ip2_RowIndex,
                                                ip2_ColumnIndex, dp2_HessianValue, numOfNonZerosInHessianValue);

  return returnValue;
}

int HessianRecovery::DirectRecover_SparseSolversFormat(GraphColoringInterface *g, double **dp2_CompressedMatrix,
                                                       unsigned int **uip2_HessianSparsityPattern,
                                                       unsigned int **ip2_RowIndex, unsigned int **ip2_ColumnIndex,
                                                       double **dp2_HessianValue)
{
  int returnValue = DirectRecover_SparseSolversFormat_unmanaged(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern,
                                                                ip2_RowIndex, ip2_ColumnIndex, dp2_HessianValue);
...
*/
int HessianRecovery::DirectRecover_CoordinateFormat_vectors(GraphColoringInterface *g, double **dp2_CompressedMatrix,
                                                            unsigned int **uip2_HessianSparsityPattern,
                                                            ...
                                                            vector<double> &HessianValue)
{
  ...
  for (unsigned int i = 0; i < (unsigned int)rowCount; i++)
  {
    ...
    for (unsigned int j = 1; j <= numOfNonZeros; j++)
    {
      if (uip2_HessianSparsityPattern[i][j] < i)
        continue;
...
int HessianRecovery::DirectRecover_CoordinateFormat_usermem(GraphColoringInterface *g, double **dp2_CompressedMatrix,
                                                            unsigned int **uip2_HessianSparsityPattern,
                                                            unsigned int **ip2_RowIndex, unsigned int **ip2_ColumnIndex,
                                                            double **dp2_HessianValue)
{
  ...
  int returnValue = DirectRecover_CoordinateFormat_vectors(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern,
                                                           RowIndex, ColumnIndex, HessianValue);
...
*/
int HessianRecovery::DirectRecover_CoordinateFormat_unmanaged(GraphColoringInterface *g, double **dp2_CompressedMatrix,
                                                              unsigned int **uip2_HessianSparsityPattern,
                                                              unsigned int **ip2_RowIndex,
                                                              unsigned int **ip2_ColumnIndex, double **dp2_HessianValue)
{
  if (g == NULL)
  {
    cerr << "g==NULL" << endl;
...
*/
int HessianRecovery::DirectRecover_CoordinateFormat(GraphColoringInterface *g, double **dp2_CompressedMatrix,
                                                    unsigned int **uip2_HessianSparsityPattern,
                                                    unsigned int **ip2_RowIndex, unsigned int **ip2_ColumnIndex,
                                                    double **dp2_HessianValue)
{
  int numOfNonZeros = DirectRecover_CoordinateFormat_unmanaged(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern,
                                                               ip2_RowIndex, ip2_ColumnIndex, dp2_HessianValue);
...
int HessianRecovery::IndirectRecover_RowCompressedFormat_usermem(GraphColoringInterface *g,
                                                                 double **dp2_CompressedMatrix,
                                                                 unsigned int **uip2_HessianSparsityPattern,
                                                                 double ***dp3_HessianValue)
{
  ...
  for (i = 0; i < i_SetCount; i++)
  {
    ...
    for (j = 0; j < i_SetSize; j++)
    {
      ...
      {
        vli_GroupedInducedVertexDegrees[vi_InducedVertexDegrees[i_PresentVertex]].erase(
            vlit_VertexLocations[i_PresentVertex]);
      }
      ...
      vlit_VertexLocations[i_PresentVertex] =
          vli_GroupedInducedVertexDegrees[vi_InducedVertexDegrees[i_PresentVertex]].begin();
    }
    ...
    for (j = 0; j < STEP_UP(i_HighestInducedVertexDegree); j++)
    {
      ...
      for (lit_ListIterator = vli_GroupedInducedVertexDegrees[j].begin();
           lit_ListIterator != vli_GroupedInducedVertexDegrees[j].end(); lit_ListIterator++)
      {
        if (k == STEP_DOWN(i_SetSize))
        {
          cout << STEP_UP(*lit_ListIterator) << "{" << vd_IncludedVertices[*lit_ListIterator] << "} (" << i_SetSize
               << ")" << endl;
        }
        else
        {
    ...
    for (;;)
    {
      ...
      {
        vli_GroupedInducedVertexDegrees[vi_InducedVertexDegrees[i_ParentVertex]].erase(
      ...
      vlit_VertexLocations[i_ParentVertex] =
          vli_GroupedInducedVertexDegrees[vi_InducedVertexDegrees[i_ParentVertex]].end();
      --vlit_VertexLocations[i_ParentVertex];
  ...
  for (i = 0; i < i_VertexCount; i++)
  {
    ...
    for (j = 1; j <= NumOfNonzeros; j++)
    {
      ...
      for (int k = 0; k < i_VertexDegree; k++)
      { // search through the v2i_VertexAdjacency matrix to find the correct column
        if (targetColumnID == v2i_VertexAdjacency[i][k])
        { // found it
...
int HessianRecovery::IndirectRecover_RowCompressedFormat_unmanaged(GraphColoringInterface *g,
                                                                   double **dp2_CompressedMatrix,
                                                                   unsigned int **uip2_HessianSparsityPattern,
                                                                   double ***dp3_HessianValue)
{
  ...
  *dp3_HessianValue = (double **)malloc(rowCount * sizeof(double *));
  ...
  return IndirectRecover_RowCompressedFormat_usermem(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern,
                                                     dp3_HessianValue);
}

int HessianRecovery::IndirectRecover_RowCompressedFormat(GraphColoringInterface *g, double **dp2_CompressedMatrix,
                                                         unsigned int **uip2_HessianSparsityPattern,
                                                         double ***dp3_HessianValue)
{

  int returnValue = IndirectRecover_RowCompressedFormat_unmanaged(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern,
                                                                  dp3_HessianValue);
...
int HessianRecovery::IndirectRecover_SparseSolversFormat_usermem(
    GraphColoringInterface *g, double **dp2_CompressedMatrix, unsigned int **uip2_HessianSparsityPattern,
    unsigned int **ip2_RowIndex, unsigned int **ip2_ColumnIndex, double **dp2_HessianValue,
    unsigned int numOfNonZerosInHessianValue)
{
  if (g->GetVertexColorCount() == 1)
  {
    return DirectRecover_SparseSolversFormat_usermem(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern, ip2_RowIndex,
                                                     ip2_ColumnIndex, dp2_HessianValue, numOfNonZerosInHessianValue);
  }
...
int HessianRecovery::IndirectRecover_SparseSolversFormat_unmanaged(
    GraphColoringInterface *g, double **dp2_CompressedMatrix, unsigned int **uip2_HessianSparsityPattern,
    unsigned int **ip2_RowIndex, unsigned int **ip2_ColumnIndex, double **dp2_HessianValue,
    unsigned int numOfNonZerosInHessianValue)
{
  ...
  int returnValue =
      IndirectRecover_SparseSolversFormat_usermem(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern, ip2_RowIndex,
                                                  ip2_ColumnIndex, dp2_HessianValue, numOfNonZerosInHessianValue);

  return returnValue;
}

int HessianRecovery::IndirectRecover_SparseSolversFormat(GraphColoringInterface *g, double **dp2_CompressedMatrix,
                                                         unsigned int **uip2_HessianSparsityPattern,
                                                         unsigned int **ip2_RowIndex, unsigned int **ip2_ColumnIndex,
                                                         double **dp2_HessianValue)
{

  int returnValue = IndirectRecover_SparseSolversFormat_unmanaged(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern,
                                                                  ip2_RowIndex, ip2_ColumnIndex, dp2_HessianValue);
...
int HessianRecovery::IndirectRecover_CoordinateFormat_unmanaged(
    GraphColoringInterface *g, double **dp2_CompressedMatrix, unsigned int **uip2_HessianSparsityPattern,
    unsigned int **uip2_RowIndex, unsigned int **uip2_ColumnIndex, double **dp2_HessianValue)
{
  ...
  cout << "*WriteMatrixMarket_ADOLCInput(" << s_postfix << ", 1, uip2_HessianSparsityPattern, " << g->GetVertexCount()
       << ", " << g->GetVertexCount() << ", dp2_CompressedMatrix, " << g->GetVertexCount() << ", "
       << g->GetVertexColorCount() << endl;
  WriteMatrixMarket_ADOLCInput(s_postfix, 1, uip2_HessianSparsityPattern, g->GetVertexCount(), g->GetVertexCount(),
                               dp2_CompressedMatrix, g->GetVertexCount(), g->GetVertexColorCount());
#endif
  if (g->GetVertexColorCount() == 1)
  {
    return DirectRecover_CoordinateFormat_unmanaged(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern, uip2_RowIndex,
                                                    uip2_ColumnIndex, dp2_HessianValue);
  }
  if (g == NULL)
  {
    cerr << "g==NULL" << endl;
  ...
  int returnValue = IndirectRecover_CoordinateFormat_vectors(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern,
                                                             RowIndex, ColumnIndex, HessianValue);
  ...
  ConvertCoordinateFormat2RowCompressedFormat((*uip2_RowIndex), (*uip2_ColumnIndex), (*dp2_HessianValue),
                                              g->GetVertexCount(), numOfNonZeros, dp3_Pattern, dp3_Values);

  s_postfix = "-IndirectRecover_CoordinateFormat_vectors";
  cout << "*WriteMatrixMarket_ADOLCInput(" << s_postfix << ", 2, uip2_HessianSparsityPattern, " << g->GetVertexCount()
       << ", " << g->GetVertexCount() << ", dp2_CompressedMatrix, " << g->GetVertexCount() << ", "
       << g->GetVertexColorCount() << ", dp3_Values" << endl;
  WriteMatrixMarket_ADOLCInput(s_postfix, 2, uip2_HessianSparsityPattern, g->GetVertexCount(), g->GetVertexCount(),
                               dp2_CompressedMatrix, g->GetVertexCount(), g->GetVertexColorCount(), dp3_Values);
...
int HessianRecovery::IndirectRecover_CoordinateFormat_vectors(GraphColoringInterface *g, double **dp2_CompressedMatrix,
                                                              unsigned int **uip2_HessianSparsityPattern,
                                                              vector<unsigned int> &RowIndex,
                                                              vector<unsigned int> &ColumnIndex,
                                                              vector<double> &HessianValue)
...
int HessianRecovery::IndirectRecover_CoordinateFormat_usermem(GraphColoringInterface *g, double **dp2_CompressedMatrix,
                                                              unsigned int **uip2_HessianSparsityPattern,
                                                              unsigned int **ip2_RowIndex,
                                                              unsigned int **ip2_ColumnIndex, double **dp2_HessianValue)
{
  if (g->GetVertexColorCount() == 1)
  {
    return DirectRecover_CoordinateFormat_usermem(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern, ip2_RowIndex,
                                                  ip2_ColumnIndex, dp2_HessianValue);
  }
...
int HessianRecovery::IndirectRecover_CoordinateFormat(GraphColoringInterface *g, double **dp2_CompressedMatrix,
                                                      unsigned int **uip2_HessianSparsityPattern,
                                                      unsigned int **ip2_RowIndex, unsigned int **ip2_ColumnIndex,
                                                      double **dp2_HessianValue)
{
  // #define DEBUG 5103
  int returnValue = IndirectRecover_CoordinateFormat_unmanaged(g, dp2_CompressedMatrix, uip2_HessianSparsityPattern,
                                                               ip2_RowIndex, ip2_ColumnIndex, dp2_HessianValue);
