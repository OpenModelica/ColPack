...
BipartiteGraphPartialOrdering::BipartiteGraphPartialOrdering() { Clear(); }
...
#pragma omp parallel for default(none) schedule(static)                                                                \
    shared(B, d, i_LeftVertexCount, i_MaxDegree_Private, i_MinDegree_Private) firstprivate(vi_Visited)
#endif
  for (int v = 0; v < i_LeftVertexCount; v++)
  {
    ...
    i_thread_num = 0;
#endif
    if (i_MaxDegree_Private[i_thread_num] < d[v])
      i_MaxDegree_Private[i_thread_num] = d[v];
  ...
  for (int i = 0; i < i_MaxNumThreads; i++)
  {
    if (i_MaxDegree < i_MaxDegree_Private[i])
      i_MaxDegree = i_MaxDegree_Private[i];
...
#ifdef _OPENMP
#pragma omp parallel for default(none) schedule(static)                                                                \
    shared(i_MaxNumThreads, i_NumOfVerticesToBeColored, B, delta, VertexThreadGroup, d, cout, i_MaxDegree,             \
               i_MaxDegree_Private) firstprivate(vi_Visited)
#endif
  for (int k = 0; k < i_MaxNumThreads; k++)
  {
    // reset vi_Visited
    for (size_t i = 0; i < vi_Visited.size(); i++)
      vi_Visited[i] = _UNKNOWN;
    ...
    while (i_NumOfVerticesToBeColored > 0)
    {
      ...
      // 				cout<<'*'<<endl;
      if (delta[i_thread_num] != 0 && B[i_thread_num][delta[i_thread_num] - 1].size() != 0)
        delta[i_thread_num]--;
      ...
      for (int l = m_vi_LeftVertices[v]; l < m_vi_LeftVertices[v + 1]; l++)
      {
        ...
        for (int m = m_vi_RightVertices[i_D1Neighbor]; m < m_vi_RightVertices[i_D1Neighbor + 1]; m++)
        {
          ...
          if (VertexThreadGroup[w] != i_thread_num || vi_Visited[w] == v || d[w] < 1 || w == v)
            continue;
          ...
          // 						cout<<"find w="<<w<<endl;
          while (i_w_location >= 0 && B[i_thread_num][d[w]][i_w_location] != w)
            i_w_location--;
          // 						if(i_w_location<0) {
          // 							cout<<"*** i_w_location<0"<<endl<<flush;
          // 						}
          // 						cout<<"i_w_location after="<<i_w_location<<endl;
          if (i_w_location != (((int)B[i_thread_num][d[w]].size()) - 1))
            B[i_thread_num][d[w]][i_w_location] = B[i_thread_num][d[w]][B[i_thread_num][d[w]].size() - 1];
...
#ifdef _OPENMP
#pragma omp parallel for default(none) schedule(static)                                                                \
    shared(B, d, i_RightVertexCount, i_MaxDegree_Private, i_MinDegree_Private) firstprivate(vi_Visited)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for default(none) schedule(static)                                                                \
    shared(i_LeftVertexCount, i_MaxNumThreads, i_NumOfVerticesToBeColored, B, delta, VertexThreadGroup, d, cout,       \
               i_MaxDegree, i_MaxDegree_Private) firstprivate(vi_Visited)
#endif
  for (int k = 0; k < i_MaxNumThreads; k++)
  {
    // reset vi_Visited
    ...
    while (i_NumOfVerticesToBeColored > 0)
    {
      ...
      for (int l = m_vi_RightVertices[v]; l < m_vi_RightVertices[v + 1]; l++)
      {
        ...
        for (int m = m_vi_LeftVertices[i_D1Neighbor]; m < m_vi_LeftVertices[i_D1Neighbor + 1]; m++)
        {
          ...
          // 						}
          // 						cout<<"i_w_location after="<<i_w_location<<endl;
          if (i_w_location != (((signed)B[i_thread_num][d[w]].size()) - 1))
            B[i_thread_num][d[w]][i_w_location] = B[i_thread_num][d[w]][B[i_thread_num][d[w]].size() - 1];
          B[i_thread_num][d[w]].pop_back();
...
string BipartiteGraphPartialOrdering::GetVertexOrderingVariant()
{
  ...
  else if (m_s_VertexOrderingVariant.compare("COLUMN_NATURAL") == 0)
  {
    return ("Column Natural");
  }
  else if (m_s_VertexOrderingVariant.compare("ROW_LARGEST_FIRST") == 0)
  {
    return ("Row Largest First");
  }
  else if (m_s_VertexOrderingVariant.compare("COLUMN_LARGEST_FIRST") == 0)
  {
    return ("Column Largest First");
  }
  else if (m_s_VertexOrderingVariant.compare("ROW_SMALLEST_LAST") == 0)
  {
    return ("Row Smallest Last");
  }
  else if (m_s_VertexOrderingVariant.compare("COLUMN_SMALLEST_LAST") == 0)
  {
    return ("Column Smallest Last");
  }
  else if (m_s_VertexOrderingVariant.compare("ROW_INCIDENCE_DEGREE") == 0)
  {
    return ("Row Incidence Degree");
  }
  else if (m_s_VertexOrderingVariant.compare("COLUMN_INCIDENCE_DEGREE") == 0)
  {
...
void BipartiteGraphPartialOrdering::GetOrderedVertices(vector<int> &output) { output = (m_vi_OrderedVertices); }
...
int BipartiteGraphPartialOrdering::OrderVertices(string s_OrderingVariant, string s_ColoringVariant)
{
  ...
  else if (s_ColoringVariant == "COLUMN_PARTIAL_DISTANCE_TWO")
  {
    if ((s_OrderingVariant.compare("NATURAL") == 0))
    {
  ...
  {
    cerr << endl;
    cerr << "Invalid s_ColoringVariant = \"" << s_ColoringVariant
         << "\", must be either \"COLUMN_PARTIAL_DISTANCE_TWO\" or \"ROW_PARTIAL_DISTANCE_TWO\".";
    cerr << endl;
  }
...
double BipartiteGraphPartialOrdering::GetVertexOrderingTime() { return m_d_OrderingTime; }
