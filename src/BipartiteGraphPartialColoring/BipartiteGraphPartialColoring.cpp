...
int BipartiteGraphPartialColoring::CheckVertexColoring(string s_VertexColoringVariant)
{
  ...
  else if (m_s_VertexColoringVariant.compare("COLUMN_PARTIAL_DISTANCE_TWO") == 0)
  {
    if (m_s_VertexOrderingVariant.empty())
    {
...
#pragma omp parallel for default(none) schedule(dynamic)                                                               \
    shared(cout, i_NumOfVerticesToBeColored, vi_VerticesToBeColored) firstprivate(vi_forbiddenColors)
#endif
    for (int i = 0; i < i_NumOfVerticesToBeColored; i++)
    {
      ...
      {
        i_cadidateColor = 0;
        while (vi_forbiddenColors[i_cadidateColor] == v)
          i_cadidateColor++;
      }
...
#ifdef _OPENMP
#pragma omp parallel for default(none) schedule(dynamic)                                                               \
    shared(i_NumOfVerticesToBeColored, vi_VerticesToBeColored, vi_verticesNeedNewColor, cout) private(cont)
#endif
    for (int i = 0; i < i_NumOfVerticesToBeColored; i++)
    {
...
int BipartiteGraphPartialColoring::PartialDistanceTwoColumnColoring_OMP()
{
  ...
  while (i_NumOfVerticesToBeColored != 0)
  {
    ...
    // Algo 4 - Line 4: for each right vertex v in U (in parallel) do
#ifdef _OPENMP
#pragma omp parallel for default(none) schedule(dynamic) shared(i_NumOfVerticesToBeColored, vi_VerticesToBeColored)    \
  ...
  // for each of left vertices, find its D1 neighbour (right vertices)
  {
    for (int j = m_vi_LeftVertices[i]; j < m_vi_LeftVertices[i + 1]; j++)
    {
      ...
      // for each of the right vertices, find its D1 neighbour (left vertices exclude the original left)
      {
        ...
        {
          cout << "Left vertices " << i + 1 << " and " << m_vi_Edges[k] + 1 << " (connected by right vectex "
               << m_vi_Edges[j] + 1 << ") have the same color (" << m_vi_LeftVertexColors[i] << ")" << endl;
  ...
  // for each of right vertices, find its D1 neighbour (left vertices)
  {
    for (int j = m_vi_RightVertices[i]; j < m_vi_RightVertices[i + 1]; j++)
    {
      ...
      // for each of the left vertices, find its D1 neighbour (right vertices exclude the original right)
      {
        ...
        {
          cout << "Right vertices " << i + 1 << " and " << m_vi_Edges[k] + 1 << " (connected by left vectex "
               << m_vi_Edges[j] + 1 << ") have the same color (" << m_vi_RightVertexColors[i] << ")" << endl;
          return _FALSE;
        }
      }
    }
...
int BipartiteGraphPartialColoring::GetLeftVertexColorCount()
{
  if (m_i_LeftVertexColorCount < 0 && GetVertexColoringVariant() == "Row Partial Distance Two")
  {
    ...
    {
      if (m_i_LeftVertexColorCount < m_vi_LeftVertexColors[i])
        m_i_LeftVertexColorCount = m_vi_LeftVertexColors[i];
    }
  }
...
int BipartiteGraphPartialColoring::GetRightVertexColorCount()
{
  if (m_i_RightVertexColorCount < 0 && GetVertexColoringVariant() == "Column Partial Distance Two")
  {
    ...
    {
      if (m_i_RightVertexColorCount < m_vi_RightVertexColors[i])
        m_i_RightVertexColorCount = m_vi_RightVertexColors[i];
    }
  }
...
int BipartiteGraphPartialColoring::GetVertexColorCount()
{
  if (m_i_VertexColorCount < 0 && GetVertexColoringVariant() != "Unknown")
  {
    ...
    { // GetVertexColoringVariant() == "Column Partial Distance Two"
...
void BipartiteGraphPartialColoring::GetLeftVertexColors(vector<int> &output) { output = (m_vi_LeftVertexColors); }

// Public Function 2464
void BipartiteGraphPartialColoring::GetRightVertexColors(vector<int> &output) { output = (m_vi_RightVertexColors); }
...
void BipartiteGraphPartialColoring::PrintRowPartialColoringMetrics()
{
  ...
  cout << endl;
  cout << GetVertexColoringVariant() << " Bicoloring | " << GetVertexOrderingVariant() << " Ordering | " << s_InputFile
       << endl;
  cout << endl;

  cout << endl;
  cout << "[Total Row Colors = " << STEP_UP(m_i_VertexColorCount) << "; Violation Count = " << m_i_ViolationCount << "]"
       << endl;
  cout << "[Row Vertex Count = " << STEP_DOWN(m_vi_LeftVertices.size())
       << "; Column Vertex Count = " << STEP_DOWN(m_vi_RightVertices.size()) << endl;
  cout << "[Ordering Time = " << m_d_OrderingTime << "; Coloring Time = " << m_d_ColoringTime
       << "; Checking Time = " << m_d_CheckingTime << "]" << endl;
  cout << endl;
}
...
void BipartiteGraphPartialColoring::PrintColumnPartialColoringMetrics()
{
  ...
  cout << endl;
  cout << "[Total Column Colors = " << STEP_UP(m_i_VertexColorCount) << "; Violation Count = " << m_i_ViolationCount
       << "]" << endl;
  cout << "[Row Vertex Count = " << STEP_DOWN(m_vi_LeftVertices.size())
       << "; Column Vertex Count = " << STEP_DOWN(m_vi_RightVertices.size()) << endl;
  cout << "[Ordering Time = " << m_d_OrderingTime << "; Coloring Time = " << m_d_ColoringTime
...
void BipartiteGraphPartialColoring::PrintVertexPartialColorClasses()
{
  ...
  {
    cout << endl;
    cout << "Vertex Partial Color Classes | " << m_s_VertexColoringVariant << " Coloring | "
         << m_s_VertexOrderingVariant << " Ordering | " << m_s_InputFile << " | Vertex Partial Colors Not Set" << endl;
    cout << endl;
...
double BipartiteGraphPartialColoring::GetVertexColoringTime() { return m_d_ColoringTime; }
