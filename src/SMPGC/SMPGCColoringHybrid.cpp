...
int SMPGCColoring::D1_OMP_HBJP(int nT, int &colors, vector<int> &vtxColors, const int option, const int switch_iter,
                               const int local_order)
{
  if (nT <= 0)
  {
    printf("Warning, number of threads changed from %d to 1\n", nT);
    nT = 1;
  }
...
#pragma omp parallel reduction(+ : n_uncolored)
    {
      ...
      case ORDER_SMALLEST_LAST:
        ...
        break;
      case ORDER_NATURAL:
        local_natural_ordering(Q);
        break;
      case ORDER_RANDOM:
        local_random_ordering(Q);
        break;
      default:
...
#pragma omp barrier
  ...
  {
  case HYBRID_GM3P:
    hybrid_GM3P(nT, vtxColors, QQ, local_order);
    break;
  case HYBRID_GMMP:
    hybrid_GMMP(nT, vtxColors, QQ, local_order);
    break;
  case HYBRID_SERIAL:
    hybrid_Serial(vtxColors, QQ, local_order);
    break;
  case HYBRID_STREAM:
  default:
...
int SMPGCColoring::D1_OMP_HBMTJP(int nT, int &colors, vector<int> &vtxColors, const int option, const int switch_iter,
                                 const int local_order)
{
  if (nT <= 0)
  {
...
#pragma omp parallel reduction(+ : uncolored_nodes)
    {
      ...
      for (int i = 0; i < (signed)Q.size(); i++)
      {
        ...
        for (int i = 0; i < HASH_NUM_HASH; i++)
          vwt[i] = mhash(v, HASH_SEED + HASH_SHIFT * i);
        ...
        for (auto iw = vtxPtr[v]; iw != vtxPtr[v + 1]; iw++)
        {
          ...
          for (int i = 0; i < HASH_NUM_HASH; i++)
          {
            ...
            if ((b_visdomain & (0x1 << (i << 1))) && (vwt[i] <= ww))
              b_visdomain ^= (0x1 << (i << 1));
            if ((b_visdomain & (0x2 << (i << 1))) && (vwt[i] >= ww))
              b_visdomain ^= (0x2 << (i << 1));
          }
          if (b_visdomain == 0)
            break;
