...
// ============================================================================
void SMPGCGraph::do_read_MM_struct(const string &graph_name, vector<int> &ia, vector<int> &ja, int *pMaxDeg,
                                   int *pMinDeg, double *pAvgDeg, double *iotime)
{
  if (graph_name.empty())
  {
    printf("Error! SMPGCCore() tried to read a graph with empty name.\n");
    exit(1);
  }
  ...
  {
    *iotime = 0;
    *(clock_t *)iotime = -clock();
  }
  ifstream in(graph_name.c_str());
  if (!in.is_open())
  {
    printf("Error! SMPGCCore() cannot open \"%s\".\n", graph_name.c_str());
    exit(1);
  }
  ...
  { // coordinate, array
    printf("Error! SMPGCGraph() read \"%s\" is a dense graph. Dense graph is a complete graph. Its chromatic number "
           "will be simply N+1.\n",
           graph_name.c_str());
    exit(1);
  }
  if (!(iss >> word) || word == "complex")
  { // complex, integer, real, pattern
    printf("Warning! SMPGCGraph() graph \"%s\" is a complex matrix. Only non-zero structure will be keeped.\n",
           graph_name.c_str());
  }
  if (!(iss >> word) || word == "general")
  { // general, symmetric, hermitan, skew-symmetric
    bSymmetric = false;
    printf("Warning! SMPGCGraph() grpah \"%s\" is not symmetric. The upper triangular and diagonal elements are going "
           "to be removed. \n",
           graph_name.c_str());
  }
  ...
  if (!in)
  {
    printf("Error! SMPGCCore() cannot get graph \"%s\" dimension. You should make sure it is at least \"Structural "
           "Symmetric\"\n",
           graph_name.c_str());
    exit(1);
  }
  ...
  for (int i = 0; i < row_expect; i++)
  {
    auto it = G.find(i);
    if (it != G.end())
      ja.insert(ja.end(), (it->second).begin(), (it->second).end());
    ia.push_back(ja.size());
  }
  ...
  if (pAvgDeg)
    *pAvgDeg = 1.0 * (ja.size()) / (ia.size() - 1);

  if (iotime)
  {
    *(clock_t *)iotime += clock();
    *iotime = double(*((clock_t *)iotime)) / CLOCKS_PER_SEC;
  }
  return;
}
...
// Note: store as sparsed CSR format
// ============================================================================
void SMPGCGraph::do_read_Metis_struct(const string &graph_name, vector<int> &ia, vector<int> &ja, int *pMaxDeg,
                                      int *pMinDeg, double *pAvgDeg, double *iotime)
{
  if (graph_name.empty())
  {
  ...
  {
    printf(
        "Error! SMPGCCore() cannot read metis graph \"%s\" with head '%s', because the graph has weight. The programer "
        "is too lazy to handle such situation. Please contact the author to added the support of such format.\n",
        graph_name.c_str(), line.c_str());
    exit(1);
  }
  ...
  {
    printf("Error! graph \"%s\" expected has %d vertices and entry of 2*%d neighbors, but we have only found %d "
           "vertices with %d neighbor entries. Check the file.\n",
