...
int SMPGCColoring::D1_OMP_GM3P_BIT(int nT, int &colors, vector<int> &vtxColors, const int local_order)
{
  ...
  if (sizeof(unsigned long long int) != 8)
    printf("Warning! ForbiddenArray was configured 64bit, but system cannot set up 64 bit variables for the buffer. "
           "undefined behaviors may occurs!\n");
#else
  if (sizeof(unsigned int) != 4)
    printf("Warning! ForbiddenArray was configured 32bit, but system cannot set up a 32bit variable for the buffer. "
           "undefined behaviors may occurs.\n");
#endif
