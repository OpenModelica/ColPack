...
// Construction
// ============================================================================
SMPGCOrdering::SMPGCOrdering(const string &graph_name, const string &fmt, double *iotime,
                             const string &order = "NATURAL", double *ordtime = nullptr)
...
void SMPGCOrdering::global_ordering(const string &order = "NATURAL", double *ordtime = nullptr)
{
  ...
  {
    *(time_t *)ordtime += clock();
    *ordtime = (double)(*(time_t *)ordtime) / CLOCKS_PER_SEC;
  }
}
...
void SMPGCOrdering::global_largest_degree_first_ordering()
{
  ...
  {
    m_global_ordered_vertex.insert(m_global_ordered_vertex.end(), GroupedVertexDegree[d].begin(),
                                   GroupedVertexDegree[d].end());
  }
...
// ============================================================================
void SMPGCOrdering::local_natural_ordering(vector<int> &vtxs) { sort(vtxs.begin(), vtxs.end()); }
  ...
  while (vtxs.size() != Nloc)
  {
    ...
    for (auto vit = prev_vtxs_size; vit < vtxs.size(); vit++)
    {
      ...
      for (auto wit = verPtr[v], witEnd = verPtr[v + 1]; wit < witEnd; wit++)
      {
        ...
        if (deg <= 0)
        { // <0 means w is not local, or have deleted; =0 should not happe
          continue;
        }
