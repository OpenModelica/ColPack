...
int WriteMatrixMarket_ADOLCInput(string s_postfix, int i_mode, ...)
{
  ...
  else if (i_mode == 1)
  {
    ...
    out_CompressedMatrix << i_CompressedMatrix_Row << " " << i_CompressedMatrix_Col << " "
                         << i_CompressedMatrix_Row * i_CompressedMatrix_Col << endl;
...
int buildDotWithColor(map<int, map<int, bool>> *graph, vector<int> *vi_VertexColors, vector<string> &ListOfColors,
                      string fileName)
{
...
{
  if (s_InputFile.size() == 0 || s_InputFile == "")
    s_InputFile = "list_of_colors.txt";
...
int buildDotWithoutColor(ColPack::BipartiteGraphPartialColoringInterface &g, vector<string> &ListOfColors,
                         string fileName)
{
...
int buildDotWithColor(ColPack::BipartiteGraphPartialColoringInterface &g, vector<string> &ListOfColors, string fileName)
{
  ...
  for (int i = 0; i < i_VertexCount; i++)
  {
    ...
    if (m_vi_LeftVertexColors.size() > 0)
    {
      ...
      line =
          line + "v" + itoa(i) + "_c" + itoa(m_vi_LeftVertexColors[i]) + " [style=filled fillcolor=" + color_str + "]";
    }
    else
    {
  ...
  for (int i = 0; i < i_RightVertexCount; i++)
  {
    ...
    if (m_vi_RightVertexColors.size() > 0)
    {
      ...
      // v2_c4 [color=aliceblue]
      line = line + "v" + itoa(i + i_VertexCount) + "_c" + itoa(m_vi_RightVertexColors[i]) +
             " [style=filled fillcolor=" + color_str + "]";
    }
    else
    {
      color_str = ListOfColors[0];
  ...
  for (int i = 0; i < i_VertexCount; i++)
  {
    for (int j = m_vi_Vertices[i]; j < m_vi_Vertices[i + 1]; j++)
    {
      ...
      if (m_vi_ConflictEdges.size() > 0 && m_vi_ConflictEdges[j])
      { // make the line bolder if the edge is conflict
...
{
  cerr << "Function to be built! int buildDotWithoutColor(ColPack::BipartiteGraphBicoloringInterface &g, "
...
{
  cerr << "Function to be built! int buildDotWithColor(ColPack::BipartiteGraphBicoloringInterface &g, vector<string> "
...
bool isValidOrdering(vector<int> &ordering, int offset)
{
  ...
  for (int i = 0; i < (int)ordering.size(); i++)
  {
    ...
    {
      cerr << "This vertex id " << orderingNum << " has been seen before at ordering[" << index[orderingNum]
           << "] and  ordering[" << i << "]. We have duplication!" << endl;
      return false;
    }
...
int ReadRowCompressedFormat(string s_InputFile, unsigned int ***uip3_SparsityPattern, int &rowCount, int &columnCount)
{
  ...
  if (nz_counter < nonzeros)
  { // nz_counter should be == nonzeros
    cerr << "* WARNING: ReadRowCompressedFormat()" << endl;
...
int ConvertRowCompressedFormat2SparseSolversFormat_StructureOnly(unsigned int **uip2_HessianSparsityPattern,
                                                                 unsigned int ui_rowCount, unsigned int **ip2_RowIndex,
...
int ConvertCoordinateFormat2RowCompressedFormat(unsigned int *uip1_RowIndex, unsigned int *uip1_ColumnIndex,
                                                ...
                                                unsigned int ***dp3_Pattern, double ***dp3_Values)
{
  ...
  for (int i = 1; i < i_NonZeroCount; i++)
  {
    ...
    { // uip1_RowIndex[i] == uip1_RowIndex[i-1]
      count++;
    }
  }
...
int Times2Plus1point5(double **dp2_Values, int i_RowCount, int i_ColumnCount)
{
  for (int i = 0; i < i_RowCount; i++)
  {
    ...
    {
      if (dp2_Values[i][j] != 0.)
        dp2_Values[i][j] = dp2_Values[i][j] * 2 + 1.5; // for each non-zero entry in the matrix, do the manipulation.
    }
  }
  return 0;
}
int Times2(double **dp2_Values, int i_RowCount, int i_ColumnCount)
{
  for (int i = 0; i < i_RowCount; i++)
  {
    ...
    {
      if (dp2_Values[i][j] != 0.)
        dp2_Values[i][j] = dp2_Values[i][j] * 2;
    }
  }
  return 0;
}
...
int GenerateValuesForSymmetricMatrix(unsigned int **uip2_SparsityPattern, int rowCount, double ***dp3_Value)
{
  ...
  for (unsigned int i = 0; i < (unsigned int)rowCount; i++)
  {
    ...
    for (unsigned int j = 1; j <= numOfNonZeros; j++)
    {
      if (uip2_SparsityPattern[i][j] > i)
        break;
      (*dp3_Value)[i][j] = (rand() % 2001 - 1000) / 1000.0;
      nnzCount[i]++;
      if (uip2_SparsityPattern[i][j] < i)
      { // copy the value from the low triangular to the upper triangular
        (*dp3_Value)[uip2_SparsityPattern[i][j]][nnzCount[uip2_SparsityPattern[i][j]] + 1] = (*dp3_Value)[i][j];
        nnzCount[uip2_SparsityPattern[i][j]]++;
      }
      // printf("(*dp3_Value)[%d][%d] = (%d % 2001 - 1000)/1000.0 = %7.2f \n",i,j,rand(),(*dp3_Value)[i][j]);
    }
  }
...
int ConvertRowCompressedFormat2ADIC(unsigned int **uip2_SparsityPattern_RowCompressedFormat, int i_rowCount,
                                    double **dp2_Value, std::list<std::set<int>> &lsi_valsetlist,
...
int ConvertRowCompressedFormat2CSR(unsigned int **uip2_SparsityPattern_RowCompressedFormat, int i_rowCount,
...
int ConvertMatrixMarketFormat2RowCompressedFormat(string s_InputFile, unsigned int ***uip3_SparsityPattern,
                                                  double ***dp3_Value, int &rowCount, int &columnCount)
{
  ...
  {
    printf("\t Read graph struture only. Values will NOT be read. dp3_Value will NOT be allocated memory, so don't try "
           "to use it!!!\n");
    Pause();
  }
    ...
    if (line != "")
    {
      ...
      if (b_symmetric)
      {
        ...
        { // rowIndex < colIndex
          ...
          cerr << "\t Found a nonzero in the upper triangular. A symmetric Matrix Market file format should only "
                  "specify the nonzeros in the lower triangular."
               << endl;
          exit(-1);
        }
      }
...
int MatrixMultiplication_VxS(unsigned int **uip3_SparsityPattern, double **dp3_Value, int rowCount, int columnCount,
                             double **dp2_seed, int colorCount, double ***dp3_CompressedMatrix)
{
  ...
  for (unsigned int i = 0; i < (unsigned int)rowCount; i++)
  {
    ...
    for (unsigned int j = 1; j <= numOfNonZeros; j++)
    {
      for (unsigned int k = 0; k < (unsigned int)colorCount; k++)
      {
        ...
        cout << "\trowCount=" << rowCount << "; numOfNonZeros=" << numOfNonZeros << "; colorCount=" << colorCount
             << endl;
        if (i == 256 && j == 1 && k == 0)
        {
          cout << "blah" << endl;
...
int MatrixMultiplication_SxV(unsigned int **uip3_SparsityPattern, double **dp3_Value, int rowCount, int columnCount,
                             double **dp2_seed, int colorCount, double ***dp3_CompressedMatrix)
{
...
bool ADICMatricesAreEqual(std::list<std::vector<double>> &lvd_Value, std::list<std::vector<double>> &lvd_NewValue,
                          bool compare_exact, bool print_all)
{
  double ratio = 1.;
    ...
    for (unsigned int j = 0; j < numOfNonZeros; j++)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            printf("At row %d, column %d, (*lvdi_Value)[j](%f) != (*lvdi_NewValue)[j](%f) ; (*lvdi_Value)[j] / "
                   "(*lvdi_NewValue)[j]=%f\n",
...
bool CompressedRowMatricesAreEqual(double **dp3_Value, double **dp3_NewValue, int rowCount, bool compare_exact,
                                   bool print_all)
{
  double ratio = 1.;
  int none_equal_count = 0;
  ...
  for (unsigned int i = 0; i < (unsigned int)rowCount; i++)
  {
    ...
    if (numOfNonZeros != (unsigned int)dp3_NewValue[i][0])
    {
      printf("Number of non-zeros in row %d are not equal. dp3_Value[i][0] = %d; dp3_NewValue[i][0] = %d; \n", i,
             (unsigned int)dp3_Value[i][0], (unsigned int)dp3_NewValue[i][0]);
      if (print_all)
      {
        none_equal_count++;
        continue;
    ...
    for (unsigned int j = 0; j <= numOfNonZeros; j++)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            printf("At row %d, column %d, dp3_Value[i][j](%f) != dp3_NewValue[i][j](%f) ; dp3_Value[i][j] / "
                   "dp3_NewValue[i][j]=%f\n",
                   i, j, dp3_Value[i][j], dp3_NewValue[i][j], ratio);
            cout << scientific << "    dp3_Value=" << dp3_Value[i][j] << ", dp3_NewValue=" << dp3_NewValue[i][j]
                 << endl;
            if (print_all)
            {
              none_equal_count++;
