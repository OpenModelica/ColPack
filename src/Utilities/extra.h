...
int buildDotWithColor(ColPack::BipartiteGraphPartialColoringInterface &g, vector<string> &ListOfColors,
                      string fileName);
...
*/
bool CompressedRowMatricesAreEqual(double **dp3_Value, double **dp3_NewValue, int rowCount, bool compare_exact = 1,
                                   bool print_all = 0);
...
int displayGraph(map<int, map<int, bool>> *graph, vector<int> *vi_VertexColors = NULL, int i_RunInBackground = false,
                 int filter = DOT);
...
template <class T> int displayGraph(T &g, int i_RunInBackground = false, int filter = DOT)
{
...
template <class T> int diffArrays(T *array1, T *array2, int rowCount, bool compare_exact = 1, bool print_all = 0)
{
  double ratio = 0.;
...
template <class T> int diffVectors(vector<T> array1, vector<T> array2, bool compare_exact = 1, bool print_all = 0)
{
  double ratio = 0.;
  int none_equal_count = 0;
...
template <class T> void displayCompressedRowMatrix(T **xp2_Value, int rowCount, bool structureOnly = false)
{
...
template <class T> void displayMatrix(T **xp2_Value, int rowCount, int columnCount, bool structureOnly = false)
{
  cout << setw(4) << "[" << setw(3) << "\\" << "]";
  if (structureOnly)
  {
...
template <class T> void displayVector(T *xp2_Value, int size, bool structureOnly = false)
{
  if (structureOnly)
  {
...
template <class T> void displayAdjacencyMatrix(vector<vector<T>> &xp2_Value, bool structureOnly = false)
{
  unsigned int estimateColumnCount = 20;
  cout << setw(4) << "[" << setw(3) << "\\" << "]";
  if (structureOnly)
