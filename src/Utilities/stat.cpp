...
vector<string> getListOfGraphs(string location_of_graph_list)
{
  ...
  if (!input)
  {
    cout << "**ERR getListOfGraphs: " << location_of_graph_list << " is not found" << endl;
    return list;
  }
  else
    cout << "getListOfGraphs: Found file. The following graphs will be read:" << endl;
  list.clear();
  input >> temp;
  ...
  if (i == max_iteration)
  {
    cerr << "**ERR getListOfGraphs(): i==max_iteration. May be you forget to use the \"*\" to terminate the list of "
            "graphs?"
         << endl;
  }
  input.close();
...
void toFileC_forColoringBasedOrdering(string baseDir, string stat_output_suffix, bool stat_output_append,
                                      bool stat_refresh_list)
{
  ...
  for (unsigned int i = 0; i < listOfGraphs.size(); i++)
  {
    ...
    // for(int j=0;j<1;j++)
    {
      ...
      {
      case 0:
        s_ColoringVariant = "DISTANCE_ONE";
        cout << "D1 ";
        stat_out1 << "D1,";
        stat_out2 << "D1,";
        stat_out3 << "D1,";
      ...
      case 3:
        ...
        break;
      case 4:
        s_ColoringVariant = "DISTANCE_TWO";
        cout << "D2 ";
        stat_out1 << "D2,";
        stat_out2 << "D2,";
        stat_out3 << "D2,";
      ...
      // for (int k=2; k<3; k++)
      {
        ...
        {
        case 0:
          s_OrderingVariant = "NATURAL";
          cout << "NATURAL ";
          break;
        case 1:
          s_OrderingVariant = "LARGEST_FIRST";
          cout << "LARGEST_FIRST ";
          break;
        case 2:
          s_OrderingVariant = "SMALLEST_LAST";
          cout << "SMALLEST_LAST ";
          break;
        case 3:
          s_OrderingVariant = "INCIDENCE_DEGREE";
          cout << "INCIDENCE_DEGREE ";
          break;
        case 4:
          s_OrderingVariant = "DISTANCE_TWO_LARGEST_FIRST";
          cout << "DISTANCE_TWO_LARGEST_FIRST ";
          break;
        case 5:
          s_OrderingVariant = "DISTANCE_TWO_SMALLEST_LAST";
          cout << "DISTANCE_TWO_SMALLEST_LAST ";
          break;
        case 6:
          s_OrderingVariant = "DISTANCE_TWO_INCIDENCE_DEGREE";
          cout << "DISTANCE_TWO_INCIDENCE_DEGREE ";
          break;
        case 7:
          s_OrderingVariant = "DYNAMIC_LARGEST_FIRST";
          cout << "DYNAMIC_LARGEST_FIRST ";
          break;
        case 8:
          s_OrderingVariant = "RANDOM";
          cout << "RANDOM ";
          break;
        }
        cout << "Ordering " << endl;
        ...
        stat_out2 << "," << gGraph->GetVertexOrderingTime() << "," << gGraph->GetVertexColoringTime() << ","
                  << gGraph->GetVertexColoringTime() + gGraph->GetVertexOrderingTime() << flush;
        ...
        if (j == 0)
        {
          ...
          if (k == 0)
          { // only get statistics once for each graph
...
void toFileC(string baseDir, string stat_output_suffix, vector<string> Orderings, vector<string> Colorings,
             map<string, bool> stat_flags)
{
  ...
  if (stat_flags["output_append"])
  {
    ...
    {
      cout << "NumberOfColors: Append to " << (baseDir + "NumberOfColors" + "-Coloring" + stat_output_suffix + ".csv")
           << endl;
    ...
    {
      cout << "MaxBackDegree: Append to " << (baseDir + "MaxBackDegree" + "-Coloring" + stat_output_suffix + ".csv")
           << endl;
  ...
  else
  {
    if (stat_flags["NumberOfColors"])
    {
      cout << "NumberOfColors: Write to " << (baseDir + "NumberOfColors" + "-Coloring" + stat_output_suffix + ".csv")
           << endl;
      out_NumberOfColors.open((baseDir + "NumberOfColors" + "-Coloring" + stat_output_suffix + ".csv").c_str());
    }
    ...
    if (stat_flags["MaxBackDegree"])
    {
      cout << "MaxBackDegree: Write to " << (baseDir + "MaxBackDegree" + "-Coloring" + stat_output_suffix + ".csv")
           << endl;
  ...
  for (unsigned int i = 0; i < listOfGraphs.size(); i++)
  {
    ...
    for (size_t j = 0; j < Colorings.size(); j++)
    {
      ...
      if (stat_flags["NumberOfColors"])
        out_NumberOfColors << Colorings[j] << ", ";
      if (stat_flags["Time"])
        out_Time << Colorings[j] << ", ";

      File stat_file_parsor;
      stat_file_parsor.Parse(listOfGraphs[i]);
      if (stat_flags["NumberOfColors"])
        out_NumberOfColors << stat_file_parsor.GetName();
      if (stat_flags["Time"])
        out_Time << stat_file_parsor.GetName();
      if (stat_flags["MaxBackDegree"] && j == 0)
        out_MaxBackDegree << stat_file_parsor.GetName();
      ...
      for (size_t k = 0; k < Orderings.size(); k++)
      {
        ...
        if (stat_flags["NumberOfColors"])
          out_NumberOfColors << "," << gGraph->GetVertexColorCount() << flush;
        if (stat_flags["Time"])
          out_Time << "," << gGraph->GetVertexOrderingTime() << "," << gGraph->GetVertexColoringTime() << ","
                   << gGraph->GetVertexColoringTime() + gGraph->GetVertexOrderingTime() << flush;
  ...
  if (stat_flags["NumberOfColors"])
    out_NumberOfColors.close();
  if (stat_flags["Time"])
    out_Time.close();
  if (stat_flags["MaxBackDegree"])
    out_MaxBackDegree.close();
  if (stat_flags["Graph_Stat"])
...
void toFileStatisticForBipartiteGraph(string baseDir, string stat_output_suffix, map<string, bool> stat_flags)
{
  ...
  for (unsigned int i = 0; i < listOfGraphs.size(); i++)
  {
    ...
    BipartiteGraphBicoloringInterface *gGraph =
        new BipartiteGraphBicoloringInterface(SRC_FILE, listOfGraphs[i].c_str(), "AUTO_DETECTED");
    ...
    out_Graph_Stat << "," << gGraph->GetEdgeCount();
    out_Graph_Stat << ","
                   << ((double)gGraph->GetEdgeCount()) / (gGraph->GetColumnVertexCount() * gGraph->GetRowVertexCount());
...
void toFileBiC(string baseDir, string stat_output_suffix, vector<string> Orderings, vector<string> Colorings,
               map<string, bool> stat_flags)
{
  ...
  if (stat_flags["output_append"])
  {
    if (stat_flags["NumberOfColors"])
    {
      cout << "NumberOfColors: Append to " << (baseDir + "NumberOfColors" + "-BiColoring" + stat_output_suffix + ".csv")
           << endl;
  ...
  else
  {
    if (stat_flags["NumberOfColors"])
    {
      cout << "NumberOfColors: Write to " << (baseDir + "NumberOfColors" + "-BiColoring" + stat_output_suffix + ".csv")
           << endl;
      out_NumberOfColors.open((baseDir + "NumberOfColors" + "-BiColoring" + stat_output_suffix + ".csv").c_str());
    }
  ...
  for (unsigned int i = 0; i < listOfGraphs.size(); i++)
  {
    ...
    for (size_t j = 0; j < Colorings.size(); j++)
    {
      ...
      for (size_t k = 0; k < Orderings.size(); k++)
      {
        ...
        if (stat_flags["NumberOfColors"])
          out_NumberOfColors << "," << gGraph->GetLeftVertexColorCount() << "," << gGraph->GetRightVertexColorCount()
                             << "," << gGraph->GetVertexColorCount() << flush;
        if (stat_flags["Time"])
          out_Time << ',' << gGraph->GetVertexOrderingTime() << ',' << gGraph->GetVertexColoringTime() << ','
                   << gGraph->GetVertexOrderingTime() + gGraph->GetVertexColoringTime() << flush;
...
void toFileBiPC(string baseDir, string stat_output_suffix, vector<string> Orderings, vector<string> Colorings,
                map<string, bool> stat_flags)
{
  ofstream out_NumberOfColors, out_Time;
  vector<string> listOfGraphs = getListOfGraphs(baseDir + "listOfGraphs.txt");
  ...
  if (stat_flags["output_append"])
  {
    if (stat_flags["NumberOfColors"])
    {
      cout << "NumberOfColors: Append to "
           << (baseDir + "NumberOfColors" + "-PD2Coloring" + stat_output_suffix + ".csv") << endl;
  ...
  else
  {
    if (stat_flags["NumberOfColors"])
    {
      cout << "NumberOfColors: Write to " << (baseDir + "NumberOfColors" + "-PD2Coloring" + stat_output_suffix + ".csv")
           << endl;
      out_NumberOfColors.open((baseDir + "NumberOfColors" + "-PD2Coloring" + stat_output_suffix + ".csv").c_str());
    }
  ...
  for (unsigned int i = 0; i < listOfGraphs.size(); i++)
  {
    ...
    for (unsigned int j = 0; j < Colorings.size(); j++)
    {
      ...
      for (unsigned int k = 0; k < Orderings.size(); k++)
      {
        ...
        BipartiteGraphPartialColoringInterface *gGraph =
            new BipartiteGraphPartialColoringInterface(SRC_FILE, listOfGraphs[i].c_str(), "AUTO_DETECTED");
        ...
        if (stat_flags["NumberOfColors"])
        {
          ...
          if (j == 0)
            out_NumberOfColors << ',' << gGraph->GetRightVertexColorCount();
          else
            out_NumberOfColors << ',' << gGraph->GetLeftVertexColorCount();
        }
        if (stat_flags["Time"])
          out_Time << ',' << gGraph->GetVertexOrderingTime() << ',' << gGraph->GetVertexColoringTime() << ','
                   << gGraph->GetVertexOrderingTime() + gGraph->GetVertexColoringTime();
