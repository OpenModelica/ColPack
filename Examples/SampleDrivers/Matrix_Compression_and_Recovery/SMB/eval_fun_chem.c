...
double feval(double *x, int n)
{
  ...
  for (i = 0; i < nel; i++)
  {
    for (j = 0; j < 3; j++)
    {
      ...
      c[index] = cAdot[0][i][j] - kA * (q4 * cA[nde - 1][i][j] - q1 * cA[0][i][j]) *
                                      (2 * (1 + kk * cA[0][i][j]) * (1 + kk * cA[0][i][j])) /
                                      (1 + (1 + kk * cA[0][i][j]) * (1 + kk * cA[0][i][j]));
      index++;
      c[index] = cBdot[0][i][j] - kB * (q4 * cB[nde - 1][i][j] - q1 * cB[0][i][j]) *
                                      (2 * (1 + kk * cB[0][i][j]) * (1 + kk * cB[0][i][j])) /
                                      (1 + (1 + kk * cB[0][i][j]) * (1 + kk * cB[0][i][j]));
      index++;

      c[index] = cAdot[nfe][i][j] - kA * (q2 * cA[nfe - 1][i][j] + qfe * cFEA - q3 * cA[nfe][i][j]) *
                                        (2 * (1 + kk * cA[nfe][i][j]) * (1 + kk * cA[nfe][i][j])) /
                                        (1 + (1 + kk * cA[nfe][i][j]) * (1 + kk * cA[nfe][i][j]));
      index++;
      c[index] = cBdot[nfe][i][j] - kB * (q2 * cB[nfe - 1][i][j] + qfe * cFEB - q3 * cB[nfe][i][j]) *
                                        (2 * (1 + kk * cB[nfe][i][j]) * (1 + kk * cB[nfe][i][j])) /
  ...
  for (l = 1; l < nex - 1; l++)
    for (i = 0; i < nel; i++)
    {
      for (j = 0; j < 3; j++)
      {
        c[index] = cAdot[l][i][j] - kA * q1 * (cA[l - 1][i][j] - cA[l][i][j]) *
                                        (2 * (1 + kk * cA[l][i][j]) * (1 + kk * cA[l][i][j])) /
                                        (1 + (1 + kk * cA[l][i][j]) * (1 + kk * cA[l][i][j]));
        index++;
        c[index] = cBdot[l][i][j] - kB * q1 * (cB[l - 1][i][j] - cB[l][i][j]) *
                                        (2 * (1 + kk * cB[l][i][j]) * (1 + kk * cB[l][i][j])) /
  ...
  for (l = nex - 1; l < nfe; l++)
    for (i = 0; i < nel; i++)
    {
      for (j = 0; j < 3; j++)
      {
        c[index] = cAdot[l][i][j] - kA * q2 * (cA[l - 1][i][j] - cA[l][i][j]) *
                                        (2 * (1 + kk * cA[l][i][j]) * (1 + kk * cA[l][i][j])) /
                                        (1 + (1 + kk * cA[l][i][j]) * (1 + kk * cA[l][i][j]));
        index++;
        c[index] = cBdot[l][i][j] - kB * q2 * (cB[l - 1][i][j] - cB[l][i][j]) *
                                        (2 * (1 + kk * cB[l][i][j]) * (1 + kk * cB[l][i][j])) /
                                        (1 + (1 + kk * cB[l][i][j]) * (1 + kk * cB[l][i][j]));
        index++;
      }
  ...
  for (l = nfe + 1; l < nra; l++)
    for (i = 0; i < nel; i++)
    {
      for (j = 0; j < 3; j++)
      {
        c[index] = cAdot[l][i][j] - kA * q3 * (cA[l - 1][i][j] - cA[l][i][j]) *
                                        (2 * (1 + kk * cA[l][i][j]) * (1 + kk * cA[l][i][j])) /
                                        (1 + (1 + kk * cA[l][i][j]) * (1 + kk * cA[l][i][j]));
        index++;
        c[index] = cBdot[l][i][j] - kB * q3 * (cB[l - 1][i][j] - cB[l][i][j]) *
                                        (2 * (1 + kk * cB[l][i][j]) * (1 + kk * cB[l][i][j])) /
                                        (1 + (1 + kk * cB[l][i][j]) * (1 + kk * cB[l][i][j]));
        index++;
      }
  ...
  for (l = nra; l < nde; l++)
  {
    for (i = 0; i < nel; i++)
    {
      for (j = 0; j < 3; j++)
      {
        c[index] = cAdot[l][i][j] - kA * q4 * (cA[l - 1][i][j] - cA[l][i][j]) *
                                        (2 * (1 + kk * cA[l][i][j]) * (1 + kk * cA[l][i][j])) /
                                        (1 + (1 + kk * cA[l][i][j]) * (1 + kk * cA[l][i][j]));
        index++;
        c[index] = cBdot[l][i][j] - kB * q4 * (cB[l - 1][i][j] - cB[l][i][j]) *
                                        (2 * (1 + kk * cB[l][i][j]) * (1 + kk * cB[l][i][j])) /
                                        (1 + (1 + kk * cB[l][i][j]) * (1 + kk * cB[l][i][j]));
        index++;
      }
