...
int main(int argc, char *argv[])
{
  ...
  unordered_set<string> BiColor = {"IMPLICIT_COVERING__STAR_BICOLORING", "EXPLICIT_COVERING__STAR_BICOLORING",
                                   "EXPLICIT_COVERING__MODIFIED_STAR_BICOLORING",
                                   "IMPLICIT_COVERING__GREEDY_STAR_BICOLORING"};
  unordered_set<string> PartialColor = {"COLUMN_PARTIAL_DISTANCE_TWO", "ROW_PARTIAL_DISTANCE_TWO"};
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    if (!strcmp(argv[i], "-f"))
      ...
      printf("Warning: unknown input argument\"%s\"", argv[i]);
  }
  ...
  else if (PartialColor.count(methd))
  {
    if (bVerbose)
      fprintf(stdout, "\ngraph: %s\norder: %s\nmethd: %s\nPartial Distantce Two Coloring\n", fname.c_str(),
              order.c_str(), methd.c_str());
    BipartiteGraphPartialColoringInterface *p =
