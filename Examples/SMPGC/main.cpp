...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    else if (!strcmp(argv[i], "-checkd1") || !strcmp(argv[i], "--checkd1"))
      bCheck |= 1;
    else if (!strcmp(argv[i], "-checkd2") || !strcmp(argv[i], "--checkd2"))
      bCheck |= 2;

    else
      printf("Waringing, unused argument %s", argv[i]);
  }
  ...
  for (auto &fname : fnames)
  {
    ...
    if (bVerbose)
    {
      ...
      if (iotime > 60)
      {
        printf(" %d min", ((int)iotime) / 60);
        iotime = ((int)(iotime) % 60) + (iotime - (int)(iotime));
      }
      printf(" %g sec\n", iotime);
    }
    ...
    for (auto &o : orders)
    {
      ...
      if (bVerbose)
      {
        printf("global order %s ordtime", o.c_str());
        if (ordtime > 60)
        {
          printf(" %d min", ((int)ordtime) / 60);
          ordtime = ((int)(ordtime) % 60) + (ordtime - (int)(ordtime));
        }
        printf(" %g sec\n", ordtime);
      }
...
void usage()
{
  fprintf(stderr, "\n\n\nUSAGE:\n $./ColPack -f <list of gname> -o <list of orderings> -m <list of methods> [-v] --nT "
                  "<list of number threads>\n"
                  "\n"
